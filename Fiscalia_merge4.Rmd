...
warning(paste0("There are ",nrow(dplyr::filter(Base_fiscalia_v11,is.na(fech_nac_rec))),"missing cases in date of birth (were 7 in v.10)"))
  ...
  # 2022-11-01, filter
Base_fiscalia_v12<-
  dplyr::filter(Base_fiscalia_v11, !grepl("Referral|Death",motivodeegreso_mod_imp))
...
```{r join1a, echo=T, fig.align='center', message=T, error=T, eval=T}
Base_fiscalia_v13<-
Base_fiscalia_v12 %>% 
    dplyr::group_by(hash_key) %>% 
  #select the first and without ties (only one)
...
biostat3::survRate(Surv((age_offending_imp-edad_al_ing_1), !is.na(dateofbirth_imp)) ~ motivodeegreso_mod_imp_1, data=Base_fiscalia_v13) %>% dplyr::mutate_if(is.numeric,~round(.,2)) %>%  knitr::kable("markdown", caption= "Glimpse of the survival analysis, Offending from admission by Cause of Discharge of the First Treatment")
...
biostat3::survRate(Surv((age_offending_imp-edad_al_ing_1), !is.na(dateofbirth_imp)) ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v13) %>%
