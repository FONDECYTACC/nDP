...
CONS_C1_df_dup_SEP_2020 %>% 
  subset(select= c("hash_key", "fech_nac", "fech_ing", "fech_egres_imp", "dup", "ano_bd_first", "duplicates_filtered", "id_centro", "tipo_centro", "tipo_de_programa_2", "tipo_de_plan_2", "senda", "macrozona", "nombre_region", "escolaridad_rec", "estado_conyugal_2", "compromiso_biopsicosocial", "sexo_2", "edad_al_ing", "edad_al_ing_fmt", "edad_al_egres", "edad_ini_cons", "edad_ini_sus_prin" ,"edad_ini_sus_prin_grupos", "freq_cons_sus_prin", "via_adm_sus_prin_act", 
...
for (i in 1:10) {
  ...
  CONS_C1_df_dup_SEP_2020_22_d <- 
    CONS_C1_df_dup_SEP_2020_22_d %>% 
  #age at admission
    dplyr::mutate(!!log_column_name := dplyr::case_when(!is.na(dob_imp) & fech_nac!= dob_imp~ lubridate::time_length(lubridate::interval(dob_imp, !!rlang::sym(column_name)), unit="years"), 
        !is.na(dob_imp) & is.na(fech_nac)~ lubridate::time_length(lubridate::interval(dob_imp, !!rlang::sym(column_name)), unit="years"), T~ lubridate::time_length(lubridate::interval(fech_nac, !!rlang::sym(column_name)), unit="years"))) %>% 
  #age at discharge  
    dplyr::mutate(!!log_column_name2 := dplyr::case_when(!is.na(dob_imp) & fech_nac!= dob_imp~ lubridate::time_length(lubridate::interval(dob_imp, !!rlang::sym(column_name2)), unit="years"), 
        !is.na(dob_imp) & is.na(fech_nac)~ lubridate::time_length(lubridate::interval(dob_imp, !!rlang::sym(column_name2)), unit="years"), T~ lubridate::time_length(lubridate::interval(fech_nac, !!rlang::sym(column_name2)), unit="years"))) %>% 
    as.data.table()%>% 
  purrr::when(nrow(.)>nrow(CONS_C1_df_dup_SEP_2020_22) ~ stop("More cases in the new database"), ~.) 
...
sqldf("SELECT *
  ...
  x.edad_al_egres_1 <= y.age_offending_imp")
...
lubridate::time_length(lubridate::interval(dob_imp, as.Date("2019-11-13")),unit="years"), T~ age_offending_imp)) %>% 
  dplyr::group_by(hash_key) %>% 
...
Base_fiscalia_v11 %>% 
  ...
  data.frame() %>% 
  kable("markdown", caption= "Summary descvriptive of users with more than one closest offense committed after the first admission")
```
...
sqldf("SELECT *
  ...
  ON x.hash_key == y.id AND 
  x.edad_al_egres_1 <= y.age_offending_imp") #change the direction to past events
