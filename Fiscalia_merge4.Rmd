...
This new categoriztion was named as `familia_delito_rec_prof`.
...
#must use these vectors to run iterations correctly
digits <- paste0("fech_ing_num_",1:10)
digits2 <- paste0("fech_egres_num_",1:10)
j <- 0
Base_fiscalia_v11<-
Base_fiscalia_v10 %>% 
  ...
  #dplyr::rowwise() %>% 
# dplyr::mutate(across(paste0("fech_ing_num_",1:10), ~
#    sum(get(gsub("fech_ing_num_","fech_egres_num_",cur_column()))- .,na.rm=T), .names = "dias_trat"))   
 # dplyr::mutate(across(paste0("fech_ing_num_",1:10), ~ !!rlang::sym(digits2[j<<- j+1])- ., .names = "dias_trat"))
#   
#   dplyr::mutate(across(paste0("motivodeegreso_mod_imp_",1:10), ~ dplyr::case_when(.=="Therapeutic discharge"~ "Treatment completion", grepl("Early|Late|Administrative", .) &  sum(!!rlang::sym(digits2[j<<- j+1])- !!rlang::sym(digits[j<<- j]), na.rm=T)<90 ~ "Treatment non-completion (Early)",  grepl("Early|Late|Administrative", .) & sum(!!rlang::sym(digits2[j<<- j+1])- !!rlang::sym(digits[j<<- j]), na.rm=T) >=90 ~ "Treatment non-completion (Late)", grepl("Referral|Death|Ongoing", .)~ "Censored", T~NA_character_), .names = "{col}_rec")) %>% 

#table(Base_fiscalia_v11$motivodeegreso_mod_imp_rec,Base_fiscalia_v11$motivodeegreso_mod_imp, exclude=NULL)
...
```{r join1b, echo=T, fig.align='center', message=T, error=T, eval=F}
...
sqldf("SELECT *
  ...
  ON x.hash_key == y.id AND 
  x.edad_al_ing_1 >= y.age_offending_imp") #change the direction to past events
...
CONS_TOP_2022<-
  # 107307
CONS_TOP%>% 
  dplyr::left_join(subset(dplyr::mutate(dplyr::group_by(Base_fiscalia_v11b, hash_key), hash_rn=row_number())%>% ungroup(), hash_rn==1), by= c("HASH_KEY" = "hash_key"))%>% 
...
sqldf("SELECT *
  ...
  x.age_offending_imp >= y.edad_a_ap_top_num")
...
biostat3::survRate(Surv((age_offending_imp-edad_al_ing_1), !is.na(dateofbirth_imp)) ~ motivodeegreso_mod_imp_1, data=Base_fiscalia_v11) %>% dplyr::mutate_if(is.numeric,~round(.,2)) %>%  knitr::kable("markdown", caption= "Glimpse of the survival analysis, Offending from admission by Cause of Discharge of the First Treatment")

biostat3::survRate(Surv((age_offending_imp-edad_al_egres_imp), !is.na(dateofbirth_imp)) ~ motivodeegreso_mod_imp_1, data= dplyr::filter(Base_fiscalia_v11,age_offending_imp-edad_al_egres_imp>=0))  %>% dplyr::mutate_if(is.numeric,~round(.,2)) %>%  knitr::kable("markdown", caption= "Glimpse of the survival analysis, Offending from admission by Cause of Discharge of the First Treatment")



biostat3::survRate(Surv((age_offending_imp-edad_al_ing_1), !is.na(dateofbirth_imp)) ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v11) %>%
  select(-1) %>% dplyr::mutate_if(is.numeric,~round(.,2)) %>%  knitr::kable("markdown", caption= "Glimpse of the survival analysis, Offending from admission by Cause of Discharge of the First Treatment")

biostat3::survRate(Surv((age_offending_imp-edad_al_egres_imp), !is.na(dateofbirth_imp)) ~ motivodeegreso_mod_imp_rec, data= dplyr::filter(Base_fiscalia_v11,age_offending_imp-edad_al_egres_imp>=0))  %>% select(-1) %>%  dplyr::mutate_if(is.numeric,~round(.,2)) %>%  knitr::kable("markdown", caption= "Glimpse of the survival analysis, Offending from admission by Cause of Discharge of the First Treatment")
```
...
mot_egreso_fit<- survfit(Surv(age_offending_imp-edad_al_ing_1,  !is.na(dateofbirth_imp)) ~motivodeegreso_mod_imp_rec, data=dplyr::filter(Base_fiscalia_v11,age_offending_imp-edad_al_egres_imp>=0, motivodeegreso_mod_imp_rec!="Censored"),
                               type      = "kaplan-meier",
                                error     = "greenwood",
                                conf.type = "log-log") 
...
mot_egreso_fit2<- survfit(Surv(age_offending_imp-edad_al_egres_imp,  !is.na(dateofbirth_imp)) ~motivodeegreso_mod_imp_rec, data=dplyr::filter(Base_fiscalia_v11,age_offending_imp-edad_al_egres_imp>=0, motivodeegreso_mod_imp_rec!="Censored"),
                               type      = "kaplan-meier",
                                error     = "greenwood",
                                conf.type = "log-log") 
...
Base_fiscalia_v11 %>% 
  dplyr::mutate(time_to_off_from_adm=age_offending_imp-edad_al_ing_1) %>% 
