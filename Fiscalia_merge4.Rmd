...
invisible("origen_ingreso_mod numero_de_hijos_mod, add!!!!!")
...
dplyr::filter(hash_key=="1951c4d080cda8b68759d638a840d82f", caption="Example of database") %>% 
  knitr::kable("markdown")
```
...
sqldf("SELECT *
  ...
  ON x.hash_key == y.id AND 
  x. edad_al_egres_imp <= y.age_offending_imp AND y.prision_fact = '1'")
...
Base_fiscalia_v12_pris %>% 
  dplyr::select(hash_key, fech_nac, fech_nac_rec, motivodeegreso_mod_imp_rec, age_offending_imp, edad_al_ing_1, caseid, crime_code_group_rec_prof, end_type_2, gls_proctermino, s_sentence_1, s_restorative_3, s_kindprison_1) %>% 
    ...
    dplyr::summarise(total= n(),max= max(n), min= min(n), mean=round(mean(n),2), p025= quantile(n, .025), p25= quantile(n, .25), median= quantile(n, .5), p75= quantile(n, .75), p975= quantile(n, .975)) %>% 
  data.frame() %>% 
  kable("markdown", caption= "Summary descvriptives of users with more than one closest offense committed after the first admission that ended in imprisonment (in number of relationships)")
```
...
Base_fiscalia_v12_pris %>% 
  ...
  dplyr::slice_min(age_offending_imp, n = 1, with_ties = F) %>% 
  dplyr::ungroup() %>% 
  dplyr::select(hash_key, age_offending_imp)
...
sqldf("SELECT *
  ...
  ON x.hash_key == y.id AND 
  x.age_offending_imp > y.age_offending_imp") #changed the direction to past events, where age at discharge is greater than the age of commission
...
paste0("(p= ",format(nrow(dplyr::distinct(dplyr::filter(Base_fiscalia_v11, grepl("Referral|Death|Censored|Ongoing",motivodeegreso_mod_imp)),hash_key)),big.mark=","),"; RUCs= ",
           ...
           format(nrow(dplyr::filter(dplyr::filter(Base_fiscalia_v11, grepl("Referral|Death|Censored|Ongoing",motivodeegreso_mod_imp)))), big.mark=","),")")
