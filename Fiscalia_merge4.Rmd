...
format_cells <- function(df, rows ,cols, value = c("italics", "bold", "strikethrough")){
  ...
  for (r in rows){
    for(c in cols){
      ...
      # Update formatting
      df[r, c] <- ifelse(nchar(df[r, c])==0,"",paste0(markup, df[r, c], markup))
    }
  }
...
'obs' = 'obs',
'edad_comision_imp' = 'date_offending_imp',
'edad_ter_rel_imp' = 'age_offending_imp',
...
Base_fiscalia_v9<-
  Base_fiscalia_v9 %>% 
  #dplyr::select(id, caseid, victim, offender_d, victim_type_n, victim_type_c, id_victim, id_relac, reg, reg_c, end_type_group, end_type, crime_code, crime_code_c, crime_code_group, end_type_2c, end_type_2, offender_type, offender_type2, offence_type, date_offending, s_sentence_1, s__fine_1, s_restorative_3, s_kindprison_1, s_sentence_2, s__fine_2, s_restorative_2, s_kindprison_2, dateofbirth_imp, obs, date_offending_imp, age_offending_imp, crime_code_group_rec, sex_imp, country_b, country_a) %>% 
  dplyr::mutate(crime_code_group= dplyr::case_when(crime_code_group== 'CUASIDELITOS'~ 'QUASIDELITES',
crime_code_group== 'DELITOS CONTRA LA FE PÚBLICA'~ 'OFFENCES AGAINST PUBLIC FAITH',
crime_code_group== 'DELITOS CONTRA LA LIBERTAD E INTIMIDAD DE LAS PERSONAS'~ 'OFFENCES AGAINST LIBERTY AND PRIVACY D',
crime_code_group== 'DELITOS CONTRA LEYES DE PROPIEDAD INTELECTUAL E INDUSTRIAL'~ 'OFFENCES AGAINST INTELLECTUAL PROPERTY LAWS',
crime_code_group== 'DELITOS DE JUSTICIA MILITAR'~ 'MILITARY JUSTICE OFFENCES',
crime_code_group== 'DELITOS DE LEYES ESPECIALES'~ 'OFFENCES UNDER SPECIAL LAWS',
crime_code_group== 'DELITOS DE TORTURA, MALOS TRATOS, GENOCIDIO Y LESA HUMANIDAD'~ 'OFFENCES OF TORTURE, ILL-TREATMENT, GENOCIDE, ETC.',
crime_code_group== 'DELITOS ECONÓMICOS Y TRIBUTARIOS'~ 'ECONOMIC AND TAX OFFENCES',
crime_code_group== 'DELITOS FUNCIONARIOS'~ 'OFFENCES AGAINST CIVIL SERVANTS',
crime_code_group== 'DELITOS LEY DE DROGAS'~ 'DRUG LAW OFFENCES',
crime_code_group== 'DELITOS LEY DE TRÁNSITO'~ 'TRAFFIC LAW OFFENCES',
crime_code_group== 'DELITOS SEXUALES'~ 'SEXUAL OFFENCES',
crime_code_group== 'FALTAS'~ 'MISDEMEANOURS',
crime_code_group== 'HECHOS DE RELEVANCIA CRIMINAL'~ 'OFFENCES OF CRIMINAL RELEVANCE',
crime_code_group== 'HOMICIDIOS'~ 'HOMICIDES',
crime_code_group== 'HURTOS'~ 'BURGLARY',
crime_code_group== 'LESIONES'~ 'INJURIES',
crime_code_group== 'OTROS DELITOS'~ 'OTHER CRIMES',
crime_code_group== 'OTROS DELITOS CONTRA LA PROPIEDAD'~ 'OTHER CRIMES AGAINST PROPERTY',
crime_code_group== 'ROBOS'~ 'VIOLENT THEFTS',
crime_code_group== 'ROBOS NO VIOLENTOS'~ 'NON-VIOLENT THEFTS',
T~crime_code_group)) %>% 
  dplyr::mutate(s_kindprison_1=dplyr::case_when(
    s_kindprison_1== 'Presidio Mayor grado medio' ~ 'Prison Major medium grade',
    s_kindprison_1== 'Presidio Mayor grado máximo' ~ 'Prison Major maximum grade',
    s_kindprison_1== 'Presidio Mayor grado mínimo' ~ 'Prison Mayor minimum degree',
    s_kindprison_1== 'Presidio Menor grado medio' ~ 'Prison Menor medium grade',
    s_kindprison_1== 'Presidio Menor grado máximo' ~ 'Prison Menor maximum degree',
    s_kindprison_1== 'Presidio Menor grado mínimo' ~ 'Minimum minimum term of imprisonment',
...
We saw the relationships by RUN and RUC.
...
warning(paste0("There are ",length(unique(concatenated_run_ruc_n_rel_n_offenses_f1$ruc)), " rucs with more than one crime within a case"))
...
concatenated_run_ruc_n_rel_n_offenses %>% dplyr::ungroup() %>%  dplyr::mutate(n=cut2(n),n_distinct_offenses=cut2(n_distinct_offenses)) %>% dplyr::group_by(n, n_distinct_offenses) %>% count() %>% dplyr::ungroup() %>% dplyr::mutate(perc=round(nn/sum(nn)*100,2)) %>% 
  knitr::kable("markdown", col.names = c("Number of relationships by RUC", "Number of distinct offenses by RUC", "n", "%"))
```
:::
...
as.character(unlist(distinct(dplyr::filter(dplyr::ungroup(concatenated_run_ruc_n_rel_n_offenses),n>1),caseid)))) %>% 
  dplyr::select(id, dateofbirth_imp, sex_imp, country_a, caseid, id_relac, id_victim, age_offending_imp, date_offending_imp, crime_code_c, reg_c, end_type_2, s_sentence_1, s__fine_1, s_kindprison_1, s_sentence_2, s__fine_2, s_kindprison_2) %>% 
...
We checked and resolved the inconsistencies in male users recieving women-only treatments after imputation.

<br>

::: controlly
```{r pre-join, echo=T, fig.align='center', message=T, error=T, eval=T}
invisible("Exported database in 22-09-2022")
CONS_C1_df_dup_SEP_2020_22<-
CONS_C1_df_dup_SEP_2020 %>% 
  subset(select= c("hash_key", "fech_nac", "dup", "ano_bd_first", "duplicates_filtered", "id_centro", "tipo_centro", "tipo_de_programa_2", "tipo_de_plan_2", "senda", "macrozona", "nombre_region", "escolaridad_rec", "estado_conyugal_2", "compromiso_biopsicosocial", "sexo_2", "edad_al_ing", "edad_ini_cons", "edad_ini_sus_prin" ,"edad_ini_sus_prin_grupos", "freq_cons_sus_prin", "via_adm_sus_prin_act", 
...
dplyr::mutate(freq_cons_sus_prin= readr::parse_factor(as.character(freq_cons_sus_prin),levels=c('Did not use', 'Less than 1 day a week','2 to 3 days a week','4 to 6 days a week','1 day a week or more','Daily'), ordered =T,trim_ws=T,include_na =F, locale=readr::locale(encoding = "UTF-8"))) %>% 
  ...
  dplyr::mutate(fech_ing_num_discard=fech_ing_num, fech_egres_num_discard= fech_egres_num) %>% 
   tidyr::pivot_wider(
    ...
    names_sep="_",
    values_from = c(fech_ing_num_discard, fech_egres_num_discard, edad_al_ing))%>% #glimpse() 
     dplyr::group_by(hash_key)%>%
...
#set iter
digits <- paste0("fech_ing_num_",1:10)
j <- 0
...
# If there are inconsistencies in sex
  dplyr::mutate(sex= dplyr::case_when(!is.na(sex_imp) & sex_imp!=as.character(sexo_2)~ sex_imp,
                                      !is.na(sex_imp) & is.na(sexo_2)~ sex_imp,
                                      T~as.character(sexo_2))) %>% 
...
# If there are inconsistencies in the date of birth
  dplyr::mutate(fech_nac_num= as.numeric(as.Date(fech_nac))) %>% 
  dplyr::mutate(dob_imp_num= as.numeric(as.Date(dateofbirth_imp))) %>%
  ...
  dplyr::mutate(edad_al_ing_1_b= (fech_ing_num_1- as.numeric(as.Date(fech_nac)))/365.25) %>% 
  purrr::when(dplyr::filter(.,abs(edad_al_ing_1-edad_al_ing_1_b)>0.002) %>% nrow()>0 ~ stop("Age at admission was calculated differently"), ~.) %>% 
# replace year at admission
  dplyr::mutate(edad_al_ing_imp= dplyr::case_when(!is.na(dob_imp_num) & fech_nac_num!= dob_imp_num~ (fech_ing_num_1- dob_imp_num)/365.25, !is.na(dob_imp_num) & is.na(fech_nac_num)~ (fech_ing_num_1- dob_imp_num)/365.25, T~ as.numeric(edad_al_ing_1)))%>% 
# Age at admission for each treatment
  dplyr::mutate_at(vars(paste0("edad_al_ing_",1:10)), ~dplyr::case_when(!is.na(dob_imp_num) & fech_nac_num!= dob_imp_num~ ( !! rlang::sym(digits[j<<- j+1])- dob_imp_num)/365.25, !is.na(dob_imp_num) & is.na(fech_nac_num)~ ( !! rlang::sym(digits[j<<- j+1])- dob_imp_num)/365.25, T~ as.numeric(.)))%>%
# Erase columns
  dplyr::select(-sex_imp, -sexo_2, -edad_al_ing_1_b, -fech_nac, -edad_al_ing_imp) %>% 
  as.data.table()%>% 
  purrr::when(nrow(.)>nrow(CONS_C1_df_dup_SEP_2020_22) ~ stop("More cases in the new database"), ~.)
```
:::

Once standardization is complete, we joined the databases `CONS_C1_df_dup_SEP_2020_22_b` and `Base_fiscalia_v9` into a single one.  The master database was SENDAs treatments, and we only used records of POs database which had an age of offending equal or posterior to the age of admission to treatment. Hence, we can see

```{r join1, echo=T, fig.align='center', message=T, error=T, eval=T}
# #_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_

sel <- toString(setdiff(names(Base_fiscalia_v9), "sex", "dateofbirth"))

#WHERE rn_id = 1
Base_fiscalia_v10<-
sqldf("SELECT *
  FROM CONS_C1_df_dup_SEP_2020_22_b AS x  
  LEFT JOIN (SELECT *
             FROM Base_fiscalia_v9
             ) AS y
  ON x.hash_key == y.id AND 
  x.edad_al_ing_1 <= y.age_offending_imp")

paste0("Observations of SENDA database: ",nrow(CONS_C1_df_dup_SEP_2020_22_b))#109756)
...
Base_fiscalia_v10 %>% 
  ...
  #janitor::tabyl(!is.na(dob_imp_num))
  #previously recoded, 
  dplyr::select(-sex, -dateofbirth_imp, -country, -victim, -id_victim, -crime_code_c , -reg_c, -end_type_2c, -cod_comunadelito, -cod_lugarocurrencia, -date_offending_imp, -sex_imp, -region_delito, -filter, -id)%>%
  plyr::rename(c("sex_2"="sex", "dateofbirth_imp_2"="dateofbirth_imp")) %>% 
  dplyr::ungroup() %>% 
  #generates errors with survival setting
  #dplyr::mutate_at(vars(paste0("edad_al_ing_",1:10)), ~dplyr::case_when(is.na(.)~ (as.numeric(as.Date("2019-11-13"))-dob_imp_num)/365.25, T~ as.numeric(.)))%>%
  #make censorship date of age of comission
  dplyr::mutate(age_offending_imp= dplyr::case_when(is.na(age_offending_imp)~ (as.numeric(as.Date("2019-11-13"))-dob_imp_num)/365.25, T~ age_offending_imp)) %>% 
  dplyr::mutate(edad_al_egres_imp= (fech_egres_num_1-dob_imp_num)/365.25)%>% 
  dplyr::group_by(hash_key) %>% 
  dplyr::slice_min(age_offending_imp, with_ties = T) %>% 
  dplyr::ungroup() %>% 
...
invisible("Ver por qué los valores negativos")
...
```{r exp_surv1, echo=T, fig.align='center', message=T, error=T, eval=T}

biostat3::survRate(Surv((age_offending_imp-edad_al_ing_1), !is.na(dateofbirth_imp)) ~ motivodeegreso_mod_imp_1, data=Base_fiscalia_v11) %>% knitr::kable("markdown", caption= "Glimpse of the survival analysis, Offending from admission by Cause of Discharge of the First Treatment")

biostat3::survRate(Surv((age_offending_imp-edad_al_egres_imp), !is.na(dateofbirth_imp)) ~ motivodeegreso_mod_imp_1, data= dplyr::filter(Base_fiscalia_v11,age_offending_imp-edad_al_egres_imp>=0)) %>% knitr::kable("markdown", caption= "Glimpse of the survival analysis, Offending from admission by Cause of Discharge of the First Treatment")
```
...
```{r exp_surv2, echo=T, fig.align='center', message=T, error=T, eval=T}

mot_egreso_fit<- survfit(Surv(age_offending_imp-edad_al_ing_1,  !is.na(dateofbirth_imp)) ~motivodeegreso_mod_imp_1, data=dplyr::filter(Base_fiscalia_v11,age_offending_imp-edad_al_egres_imp>=0),
                               type      = "kaplan-meier",
                                error     = "greenwood",
                                conf.type = "log-log") 
...
```{r exp_surv3, echo=T, fig.align='center', message=T, error=T, eval=T}

mot_egreso_fit2<- survfit(Surv(age_offending_imp-edad_al_egres_imp,  !is.na(dateofbirth_imp)) ~motivodeegreso_mod_imp_1, data=dplyr::filter(Base_fiscalia_v11,age_offending_imp-edad_al_egres_imp>=0),
                               type      = "kaplan-meier",
                                error     = "greenwood",
                                conf.type = "log-log") 

mot_egreso_fit_na2 <- mot_egreso_fit2 %>% fortify %>% group_by(strata) %>% mutate(CumHaz = cumsum(n.event/n.risk))

ggsurvplot(mot_egreso_fit2,
           fun = "cumhaz",
           conf.int = TRUE,
           #legend.labs = c("Late Dropout", "Early Dropout"), 
           risk.table = "abs_pct",
...
# end of proceeding  
tab7_lab_aft_d2<-
    paste0("(p= ",format(nrow(dplyr::distinct(dplyr::filter(Base_fiscalia_v8, dplyr::case_when( grepl("REPARATORIO|CONDICIONAL",gls_mottermino) & is.na(agrupa_terminos)~F, grepl("REPARATORIO|SENTENCIA DEFINITIVA CONDENATORIA|240|MONIT", toupper(agrupa_terminos), ignore.case=F)~F, T~T)),rut_enc_saf)),big.mark=","),"; RUCs= ",
           format(nrow(dplyr::distinct(dplyr::filter(Base_fiscalia_v8, dplyr::case_when( grepl("REPARATORIO|CONDICIONAL",gls_mottermino) & is.na(agrupa_terminos)~F, grepl("REPARATORIO|SENTENCIA DEFINITIVA CONDENATORIA|240|MONIT", toupper(agrupa_terminos), ignore.case=F)~F, T~T)),ruc)), big.mark=","),";n= ",
           format(nrow(dplyr::filter(Base_fiscalia_v8, dplyr::case_when( grepl("REPARATORIO|CONDICIONAL",gls_mottermino) & is.na(agrupa_terminos)~F, grepl("REPARATORIO|SENTENCIA DEFINITIVA CONDENATORIA|240|MONIT", toupper(agrupa_terminos), ignore.case=F)~F, T~T))), big.mark=","),")")

#_#_#_#_#_length(unique(Base_fiscalia_v10$id))
#After join
# tab7_lab_aft_d3<-
#     paste0("(p= ",format(nrow(dplyr::distinct(dplyr::filter(Base_fiscalia_v11, ,big.mark=","),"; RUCs= ",
#            format(nrow(dplyr::distinct(dplyr::filter(Base_fiscalia_v8, dplyr::case_when( grepl("REPARATORIO|CONDICIONAL",gls_mottermino) & is.na(agrupa_terminos)~F, grepl("REPARATORIO|SENTENCIA DEFINITIVA CONDENATORIA|240|MONIT", toupper(agrupa_terminos), ignore.case=F)~F, T~T)),ruc)), big.mark=","),";n= ",
#            format(nrow(dplyr::filter(Base_fiscalia_v8, dplyr::case_when( grepl("REPARATORIO|CONDICIONAL",gls_mottermino) & is.na(agrupa_terminos)~F, grepl("REPARATORIO|SENTENCIA DEFINITIVA CONDENATORIA|240|MONIT", toupper(agrupa_terminos), ignore.case=F)~F, T~T))), big.mark=","),")")

tab7_lab_aft_d<- paste0('&#8226;Discard observations coded as victims rather than ofenders ',tab7_lab_aft_d1,'\\\\\\l&#8226;Discard observations depending on the values of the end of the proceedings ',tab7_lab_aft_d2,'          \\\\\\l&#8226;Long-to-wide relationships/crimes, end of judicial proceedings, penalty         \\\\\\l&#8226;Group crimes into violent, drug-related, etc.         \\\\\\l&#8226;Check overlapping events (e.g., incarcerated while completing a residential treatment)         \\\\\\l')
...
webshot(paste0(sub("2019 \\(github\\)/SUD_CL","2022 \\(github\\)",path),"/_figs/_flowchart_merge_222_3.html"), paste0(sub("2019 \\(github\\)/SUD_CL","2022 \\(github\\)",path),"/_figs/_flowchart_merge_formatted_3.png"),vwidth = 1200, vheight = 900,
        zoom = 2)
```
...
```{r export, warning=FALSE, echo=T, error=T, eval=F}
invisible("Exported database in 22-09-2022")
CONS_C1_df_dup_SEP_2020_22<-
CONS_C1_df_dup_SEP_2020 %>% 
