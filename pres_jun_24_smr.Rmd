---
main_topsize: 0.11 #percent coverage of the poster
...
# Background
<ul>
<li>Chile has one of the highest alcohol consumption in the continent$`r Cite(myBib, c("vicente_prevalencias_2016"))`$</li>
<li>In 2016, led in high school cocaine, coca paste, and marijuana use$`r Cite(myBib, c("inter-american_drug_abuse_control_commission_cicad_report_2019"))`$</li>
<li>Over 170% growth in drug-related mortality, 2000 vs 2019$`r Cite(myBib, c("oecd_health_2023"))`$</li>
...
<li>In Chile, the government funds substance use treatments (SUT) for all with public health insurance (~81% of the population)$`r Cite(myBib, c("mateo-pinones_evidence-based_2022"))`$</li>
...
```{r res-desc-output, eval=T, echo=F, error=F, fig.align='center', fig.cap="Characteristics of SUT patients", fig.retina=2, message=FALSE, warning=FALSE, out.height="88%", out.width="88%"}
...
.
:::
```{r smrs-plot, eval=T, echo=F, error=T, fig.align='center', fig.cap="Standardized mortality ratios for people in SUT, by age group and sex",  message=FALSE, warning=FALSE, fig.retina=2, out.height="85%", out.width="85%"}
...
ggplot(data_age_sex, aes(x = AgeGroup, y = SMD, color = Sex)) +
  ...
  scale_color_grey(start = 0.6, end = 0.4) +  # Adjust the grey scale as needed
  theme_minimal() +
  geom_hline(yintercept = 1, linetype = "dashed", color = "black", linewidth = 1)+  # Horizontal line at y=1
  ...
  labs(caption= "Y-axis was log-transformed. Dashed black lines represent the null value.")
plot_smds  
```
...
```{r chile-regional, eval=T, echo=F, error=T, fig.align='center', fig.cap="Regional data in quartiles", fig.width=10, fig.height=12, fig.retina=2}
...
combined_plot <- cowplot::plot_grid(rme_region, haz_use_oh, use_lst_yr_mar, use_lst_yr_coc, labels = c("A", "B", "C", "D"))

combined_plot <- cowplot::plot_grid(
  rme_region, haz_use_oh, use_lst_yr_mar, use_lst_yr_coc,
  labels = c("A", "B", "C", "D"),
  ncol = 2,  # Arrange in 2 columns
  align = 'hv',  # Align horizontally and vertically
    rel_widths = c(1, 1),  # Relative widths of columns
  rel_heights = c(1, 1)  # Relative heights of rows
)
ggpubr::ggarrange(rme_region, haz_use_oh, use_lst_yr_mar, use_lst_yr_coc, ncol = 2, nrow = 2,  heights = c(1, 1, 1, 1))# labels = c("A", "B", "C", "D"),
# Display the combined plot
#print(combined_plot)
#ggsave("E:/Mi unidad/Alvacast/SISTRAT 2022 (github)/_outputs/ser2024/comb_Ploot.png",combined_plot, dpi=500)
#
...
<ul>
  ...
  <li>Largest sex gap at younger ages</li>
  <li>Further exploration of substance use's contribution to elevated risk</li>
  <li>Mortality risks were higher in areas influenced by ethnic and cultural barriers, discrimination, or geographical isolation, which hinder access to treatment</li>
</ul>
