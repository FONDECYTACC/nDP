...
**We analyze Chile as a case study and examine the impact of SUT on the prevention of contact with the criminal justice system (CJS) in the middle (3 years) and long term (5 years).**

# Methods
- **Design**: a population-based record-linkage retrospective cohort design.
- **Participants**: patients were enrolled in publicly funded SUT programs in Chile, 2010-2019.
...
<li>We used Royston-Parmar survival models and adjust for the effects of other factors, and predicted standardized survival probabilities and restricted mean time lost (RMTL)$`r Cite(myBib, c("s457128"))`$.</li>
<li>Missing data was imputed$`r Cite(myBib, c("missranger"))`$.</li>
...
<li>**Imprisonment**: E-value of at least 2.36 for Early and  1.99 for Late dropout vs. treatment completion at baseline (*t=0*).</li>
...
```{r exp-surv3-prob-plot, eval=T, fig.align='center', message=F, echo=F, error=T, fig.align='center', fig.cap="", fig.retina=2, fig.cap="Differences in survival probabilities(left) and RMTLs(right) for time-to-condemnatory sentence(up) & imprisonment(bottom)"}
...
image_montage(c(fig1, fig2rmtl, fig3, fig4rmtl), tile = '2x2', geometry = 'x300+10+10',gravity = "Center")
```
...
- Effective SUT can help to reduce the harms of substance use disorders and criminality.
