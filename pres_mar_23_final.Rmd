...
logoright_name: _figs/intramural_22_23.png
logocenter_name: _figs/qrcode.png #"`r plot(qrcode::qr_code('https://fondecytacc.github.io/nDP/index_prop_grant22_23.html'))`"
output: 
...
**We analyzed Chile as a case study and examined the impact of SUT on the prevention of contact with the criminal justice system (CJS) in the middle (3 years) and long term (5 years).**
...
**Analysis plan**
<li>We calculated cumulative incidence rates and incidence rate ratios(IRR)</li>
<li>We used Royston-Parmar survival models and adjust for the effects of other factors, and predicted standardized survival probabilities and restricted mean survival times (RMST)$`r Cite(myBib, c("s457128"))`$.</li>
<li>We imputed missing data under MAR assumption$`r Cite(myBib, c("missranger"))`$.</li>
<li>We calculated e-values of the strength of confounding needed to take away the associations between treatment outcome and contact with CJS</li>
<li>Codes are available at https://fondecytacc.github.io/nDP/index_prop_grant22_23.html. Covariates are listed below:</li>
...
# Preliminary Results
Of the `r format(nrow(CONS_C1_df_dup_SEP_2020), big.mark=",")`(patients= `r format(length(unique(CONS_C1_df_dup_SEP_2020$hash_key)), big.mark=",")`) SENDA records of admissions,  `r format(nrow(Base_fiscalia_v14), big.mark=",")`(`r scales::percent(nrow(Base_fiscalia_v13)/length(unique(CONS_C1_df_dup_SEP_2020$hash_key)))`) were eligible to be matched with the Prossecutor's Office database (discarded ongoing treatments or treatments that ended in referrals). 
<li>**Condemnatory sentence**. `r format(nrow(dplyr::filter(Base_fiscalia_v14, !is.na(dateofbirth_imp))), big.mark=",")`(`r scales::percent(nrow(dplyr::filter(Base_fiscalia_v14, !is.na(dateofbirth_imp)))/nrow(Base_fiscalia_v14))`).</li>
<li>**Imprisonment**. `r format(nrow(dplyr::filter(Base_fiscalia_v14_pris, !is.na(dateofbirth_imp))), big.mark=",")`(`r scales::percent(nrow(dplyr::filter(Base_fiscalia_v14_pris, !is.na(dateofbirth_imp)))/nrow(Base_fiscalia_v14_pris))`).</li>
...
-	**Early dropout vs. Treatment completion**: Patients completing treatment took longer to an offense leading to condemnatory sentence (IRR= `r sprintf("%1.2f", irrs_early_drop$estimate[1])` 95% CI `r sprintf("%1.2f", irrs_early_drop$conf.int[1])`,`r sprintf("%1.2f", irrs_early_drop$conf.int[2])`; aHR[adjusted hazard ratio]: 1.74  95%CI 1.66, 1.83)  condemnatory sentence and imprisonment (IRR= `r sprintf("%1.2f", irrs_early_drop_pris$estimate[1])` 95% CI `r sprintf("%1.2f", irrs_early_drop_pris$conf.int[1])`,`r sprintf("%1.2f", irrs_early_drop_pris$conf.int[2])`; aHR= 1.99 95%CI 1.79, 2.22).
-	**Late dropout vs. Treatment completion**: Patients completing treatment took longer to condemnatory sentence (IRR= `r sprintf("%1.2f", irrs_late_drop$estimate[1])` 95% CI  `r sprintf("%1.2f", irrs_late_drop$conf.int[1])`,`r sprintf("%1.2f", irrs_late_drop$conf.int[2])`; aHR=1.58 95%CI 1.52, 1.65) and to imprisonment (IRR= `r sprintf("%1.2f", irrs_late_drop_pris$estimate[1])` 95% CI  `r sprintf("%1.2f", irrs_late_drop_pris$conf.int[1])`,`r sprintf("%1.2f", irrs_late_drop_pris$conf.int[2])`; aHR=1.65 95%CI 1.51, 1.81). 
-	**Late vs. Early dropout**: Patients completing treatment took longer to condemnatory sentence (IRR= `r sprintf("%1.2f", irrs_early_late_drop$estimate[1])` 95% CI `r sprintf("%1.2f", irrs_early_late_drop$conf.int[1])`,`r sprintf("%1.2f", irrs_early_late_drop$conf.int[2])`) and imprisonment (IRR= `r sprintf("%1.2f", irrs_early_late_drop_pris$estimate[1])` 95% CI  `r sprintf("%1.2f", irrs_early_late_drop_pris$conf.int[1])`,`r sprintf("%1.2f", irrs_early_late_drop_pris$conf.int[2])`). 
...
```{r exp-surv23-1, eval=F, echo=F, error=T, fig.align='center', message=FALSE, warning=FALSE}
...
rmst_imp %>% as.data.table() %>% janitor::row_to_names(3) %>% data.frame() %>% 
  ...
  dplyr::mutate_at(2:(ncol(.)-1), ~sprintf("%1.3f",readr::parse_number(.))) %>% 
...
rbindlist(list(list(c("Probs.",rep("",0))),cbind.data.frame(prob_imp_tab,prob_imp_diff_tab[,-1]),
               list(c("rmst",rep("",0))), cbind.data.frame(rmst_imp_tab,rmst_imp_diff_tab[,-1])), fill=T) %>% 
...
rbindlist(
  list(list(c("Probs.",rep("",0))),cbind.data.frame(prob_imp_pris_tab,prob_imp_diff_pris_tab[,-1]),   list(c("RMST",rep("",0))),cbind.data.frame(rmst_imp_pris_tab,rmst_imp_diff_pris_tab[,-1])), 
          fill=T) %>% 
...
```
**E-values**
<li>**Condemnatory Sentence**: E-value of at least 2.19 for Early and  2.01 for Late dropout vs. treatment completion at baseline (*t=0*).</li>
<li>**Imprisonment**: E-value of at least 2.99 for Early and 2.38 for Late dropout vs. treatment completion at baseline (*t=0*).</li>
