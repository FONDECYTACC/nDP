...
pacman::p_load(APCtools, ggpattern, withr, boot, matrixStats, knitr, tidyr, 
               ...
               sqldf,  adjustedCurves, ggpmisc, rms, irrCAC, install=T)
...
### 1.- Ver qué es Base_fiscalia_v9, por qué tiene 49,970 IDs y 174,961 filas
### 2.- CONS_C1_df_dup_SEP_2020
### 3.- 
...
We compared the records of TOP with the records of PO

```{r chisq-tot, echo=T, fig.align='center', message=T, error=T, eval=T}
...
paste0(median(Base_fiscalia_v13c_dic_2022_3$tot,na.rm=T), ' (',quantile(Base_fiscalia_v13c_dic_2022_3$tot,.25,na.rm=T),', ',quantile(Base_fiscalia_v13c_dic_2022_3$tot,.75, na.rm=T),')')

paste0(median(Base_fiscalia_v13c_dic_2022_3$tot_off_top,na.rm=T), ' (',quantile(Base_fiscalia_v13c_dic_2022_3$tot_off_top,.25,na.rm=T),', ',quantile(Base_fiscalia_v13c_dic_2022_3$tot_off_top,.75, na.rm=T),')')

cat("Spearman correlation: Total records of offenses vs. TOP reported offenses")
...
ifelse(Base_fiscalia_v13c_dic_2022_3$tot_off_top>=5,4,Base_fiscalia_v13c_dic_2022_3$tot_off_top)

cat("Confusion Matrix")
caret::confusionMatrix(factor(Base_fiscalia_v13c_dic_2022_3$tot_off_top_rec), factor(Base_fiscalia_v13c_dic_2022_3$tot))

cat("Kappa with ordinal weights")
irrCAC::kappa2.table(table(ordered(Base_fiscalia_v13c_dic_2022_3$tot_off_top_rec), ordered(Base_fiscalia_v13c_dic_2022_3$tot)),weights = irrCAC::ordinal.weights(0:4))
...
ggstatsplot::ggscatterstats(tot_off_top, tot, data=Base_fiscalia_v13c_dic_2022_3, 
                            ...
                            title="Relationship between PO and TOP")
```
...
```{r chisq_sud, echo=T, fig.align='center', message=T, error=T, eval=T, fig.show='hide'}
...
```{r roc, echo=T, fig.align='center', message=T, error=T, eval=T}
...
library(DiagrammeR) 
        ...
        tab8 -> blank65 [label= '        Selected the first row with distinct information regarding patient ID, case ID, crime code', fontsize = 8]
        
        tab8 -> blank7 [arrowhead= none]
        blank7 [label = '', width = 0.0001, height = 0.0001]
        tab9 [label= '@@9',fontsize = 7]
        tab10 [label= '@10']        
        blank7 -> tab9 
        blank7 -> tab10
                    subgraph {
          rank = same; blank7; tab9;
                    }
        ...
        # tab7 [label = '@@7',fontsize = 7]
        # tab8 [label= '@@8']
#tab1 -> blank [label= '        Selected the first row with distinct information regarding patient ID, case ID, crime code', fontsize = 8]

a_tab10_lab_aft_d<- paste0("TOP Dataset \n(n= ", formatC(nrow(CONS_TOP),big.mark = ","),";\nindividuals= ",CONS_TOP%>% dplyr::distinct(HASH_KEY)%>% nrow()%>% formatC(big.mark = ","),")")
