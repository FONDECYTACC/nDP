...
pacman::p_load(ggpattern, withr, boot, matrixStats, knitr, tidyr, 
               ...
               rpart.plot, partykit, randomForest, nn, car, WeightedROC, WeightIt, pROC, install=T)
...
#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:
  ...
  dplyr::mutate(freq_cons_sus_prin=parse_factor(as.character(freq_cons_sus_prin),levels=c('Less than 1 day a week','2 to 3 days a week','4 to 6 days a week','1 day a week or more','Daily'), ordered =T,trim_ws=F,include_na =F))
...
```{r chisq-tot-2, echo=T, fig.align='center', message=T, error=T, eval=T}
#
...
score_classifier <- function(df, predicted, actual) {
  confusion_table <- table(get(df)[[predicted]], get(df)[[actual]])
  ...
  score_df <- data.frame(accuracy = get_accuracy(df, predicted, actual),
                          classification_error_rate = get_classification_error_rate(df, predicted, actual),
                          precision = get_precision(df, predicted, actual),
                          sensitivity = get_sensitivity(df, predicted, actual),
                          specificity = get_specificity(df, predicted, actual),
                          f1_score=get_f1_score(df, predicted, actual))
return(score_df)
}
...
knitr::kable("markdown",caption= "Table 1. Properties") #%>% kableExtra::kable_classic()
...
For ROC curves, we dichotomized into committing a crime or not against reporting or not (`*_bin`). Posteriorly, we calculated confidence intervals using bootstrap with 2000 replications.

::: controlly2

```{r roc, echo=T, fig.align='center', message=T, error=T, eval=T, layout="l-body-outset"}
...
# plot(ROCit_obj)
cat("TOTAL")
roc1 <- roc(Base_fiscalia_v13c_dic_2022_3$tot_bin, Base_fiscalia_v13c_dic_2022_3$tot_off_top_bin)
roc1_auc<-ci.auc(roc1, method= "bootstrap", boot.n= n_boot)

paste0("AUC= ", round(attr(roc1_auc,"auc")[[1]],3)," (95% CI: ",round(as.numeric(roc1_auc[1]),3), ", ", round(as.numeric(roc1_auc[3]),3),")")
...
cat("Acquisitive")
roc2 <- roc(Base_fiscalia_v13c_dic_2022_3$acq_bin, Base_fiscalia_v13c_dic_2022_3$acq_top_bin)
roc2_auc<-ci.auc(roc2, method= "bootstrap", boot.n= n_boot)

paste0("AUC= ", round(attr(roc2_auc,"auc")[[1]],3)," (95% CI: ",round(as.numeric(roc2_auc[1]),3), ", ", round(as.numeric(roc2_auc[3]),3),")")

# ROCit_acq <- rocit(score=Base_fiscalia_v13c_dic_2022_3$acq_bin,class=Base_fiscalia_v13c_dic_2022_3$acq_top_bin)
# plot(ROCit_acq)
# pander::pander(ciROC(ROCit_acq,nboot = 500))

cat("Substance-related")
roc3 <- roc(Base_fiscalia_v13c_dic_2022_3$sud_bin, Base_fiscalia_v13c_dic_2022_3$sud_top_bin)
roc3_auc<-ci.auc(roc3, method= "bootstrap", boot.n= n_boot)

paste0("AUC= ", round(attr(roc3_auc,"auc")[[1]],3)," (95% CI: ",round(as.numeric(roc3_auc[1]),3), ", ", round(as.numeric(roc3_auc[3]),3),")")
...
cat("Violent")
roc4 <- roc(Base_fiscalia_v13c_dic_2022_3$vio_bin, Base_fiscalia_v13c_dic_2022_3$vio_top_bin)
roc4_auc<-ci.auc(roc4, method= "bootstrap", boot.n= n_boot)

paste0("AUC= ", round(attr(roc4_auc,"auc")[[1]],3)," (95% CI: ",round(as.numeric(roc4_auc[1]),3), ", ", round(as.numeric(roc4_auc[3]),3),")")
...
cat("Others")
roc5 <- roc(Base_fiscalia_v13c_dic_2022_3$oth_bin, Base_fiscalia_v13c_dic_2022_3$oth_top_bin)
roc5_auc<-ci.auc(roc5, method= "bootstrap", boot.n= n_boot)

paste0("AUC= ", round(attr(roc5_auc,"auc")[[1]],3)," (95% CI: ",round(as.numeric(roc5_auc[1]),3), ", ", round(as.numeric(roc5_auc[3]),3),")")
...
```{r roc-plot, echo=T, fig.align='center', message=T, error=T, eval=T, fig.cap= "ROC curves by type of crimes", layout="l-body-outset"}


# Create a basic roc object
roclist <- list("TOTAL" = roc1,
                "Acquisitive" = roc2,
                "Substance-related" = roc3,
                "Violent" = roc4,
                "Other crimes" = roc5)

g <- pROC::ggroc(roclist, aes = "linetype", legacy.axes = TRUE) +
  geom_abline() +
...
```{r an-1, eval=T, echo=T, error=T, fig.align='center', message=FALSE, warning=FALSE, paged.print=TRUE, layout="l-body-outset"}
...
```{r an-1-5, eval=F, echo=T, error=T, message=FALSE, warning=FALSE, paged.print=TRUE}
library(nnet)
library(boot)
library(furrr)

set.seed(2125) # para reproducibilidad

boot_func <- function(x) {
  boot(x %>% dplyr::mutate_if(is.numeric, ~scale(.)), fit_multinom, R = 1)$t
}

ncores <- parallel::detectCores() # detect number of cores
plan(multiprocess, workers = ncores) # set up parallel processing

boot_samples <- future_map_dbl(replicate(1000, Base_fiscalia_v13c_dic_2022_4, simplify = FALSE), boot_func) %>% t()

# Calcule los intervalos de confianza bootstrap para cada coeficiente
boot_ci <- t(future_map_dbl(1:ncol(boot_samples), function(i) {
  quantile(boot_samples[, i], c(0.025, 0.975))
}))

# Imprima los intervalos de confianza bootstrap
boot_ci

invisible("does not work")
```
