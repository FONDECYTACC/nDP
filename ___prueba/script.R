...
<br>
  ...
  ```{r paso1_dates, fig.align='center', fig.cap="Absolute differences between SENDA and POs date of birth", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
...
```{r paso1_dates2, fig.align='center', fig.cap="SENDA patients with differences in the dates of birth of SENDA DB. Number of distinct dates of comission of crimes", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
Base_fiscalia_v3 %>% 
...
```{r paso1_dates3, fig.align='center', fig.cap="SENDA patients with differences in the dates of birth of SENDA DB. Distribution of age at comission of crime", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
Base_fiscalia_v3 %>% 
  dplyr::filter(rut_enc_saf %in% unlist(fech_nac_fisc_vs_senda2)) %>% 
...
<br>
  ...
  ```{r apply_rules_age2b, echo=T, cache= T, paged.print=TRUE, eval=T, error=F}
...
cbind(n=c("Birth date","Age of commission of crime","Admission date"),
  ...
  knitr::kable(format="html",caption= paste0("Table. Summary of Dates w/ aberrant age of comission of crime (n=", nrow(dplyr::filter(Base_fiscalia_v4,edad_comision_imp<14)),")")) %>% 
  kableExtra::kable_classic(bootstrap_options = c("striped", "hover","condensed"),font_size= 12)
```

</div>
  
  ```{r discard_aberrant_age_of_comission_of_crimes, message=F, error=T, eval=T, warnings=F, echo=T}
Base_fiscalia_v5<-
  ...
  dplyr::mutate(familia_delito_rec=dplyr::case_when(grepl("LESA HUMANIDAD",familia_delito)~"VIOLENT CRIME",
                                                    ...
                                                    T~"OTHER CHARGES")) 
```
```{r discard_aberrant_age_of_comission_of_crimes2, message=F, error=T, eval=T, warnings=F, echo=T}
...
<br>
  ...
  ```{r apply_rules_age_v5_22, message=F, error=T,eval=T, warnings=F, echo=T}
rbind(
  cbind(cat="Date of comission of the crime",
        Base_fiscalia_v5 %>% 
          dplyr::summarise(min = min(fec_comision_simple, na.rm=T),
  ...
  cbind(cat="Date of birth",
        Base_fiscalia_v5 %>% 
          dplyr::summarise(min = min(imp_birth_date, na.rm=T),
                           ...
                           p975=as.Date(quantile(unclass(imp_birth_date), .975, na.rm=T), origin = "1970-01-01"),
                           max = max(imp_birth_date, na.rm=T)))) %>% 
  knitr::kable(format="html",caption= "Table. Summary of Dates (after correcting dates)") %>% #,col.names=c("Variables","Residential", "Ambulatory", "p-value")) %>% 
  kableExtra::kable_classic(bootstrap_options = c("striped", "hover","condensed"),font_size= 12) %>% 
  kableExtra::group_rows("After imputation database",1,3) %>% 
...
<br>
  ...
  ```{r fig1_agrupa_terminos_fech_nac, fig.align='center', fig.pos='H', fig.cap="Date of birth and suspicious of commiting a crime", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
...
<br>
  ...
  ```{r fig2_agrupa_terminos_fech_nac, fig.align='center', fig.pos='H', fig.cap="Date of birth and end of judicial proceedings", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
#https://coolors.co/0a1128-001f54-034078-1282a2-c1dbe3
...
```{r fig3a_sex_acc_fech_nac, fig.align='center', fig.pos='H', fig.cap="Sex, date of birth in suspicious of commiting a crime", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
#https://coolors.co/0a1128-001f54-034078-1282a2-c1dbe3
...
```{r fig3b_sex_vic_fech_nac, fig.align='center', fig.pos='H', fig.cap="Sex, date of birth in involved as a victim", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
#https://coolors.co/0a1128-001f54-034078-1282a2-c1dbe3
...
```{r fig4_fam_delito_fech_nac, fig.align='center', fig.pos='H', fig.cap="Date of birth and suspicious of commiting a crime", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
#https://coolors.co/0a1128-001f54-034078-1282a2-c1dbe3
...
```{r fig5_fam_delito_fech_nac, fig.align='center', fig.pos='H', fig.caption="Date of birth and suspicious of commiting a crime", echo=T, cache= T, paged.print=TRUE, eval=F, error=T, dpi=320, fig.align="center"}
...
Base_fiscalia_v5 %>% 
  ...
  knitr::kable(format="html",caption="Table. An example of distinct combination of charges by different criminal processes") %>% 
...
```{r 3_2_ser, echo=T, error=T, paged.print=TRUE}
font_add(family = "Rooney Sans", regular = paste0(sub("2019 \\(github\\)/SUD_CL","2022 \\(github\\)",path),"/_style/RooneySansRegular.otf"))
...
```{r 3_3_ser, echo=T, error=T, paged.print=TRUE}
...
#p.adjust(pvals, method = "BH")
 export2md(restab1_all, size=11, first.strip=T, hide.no="no", position="center",
           format="html",caption= "Table. Summary descriptives at baseline by completion status at baseline Treatments from 2010-2019")%>%#col.names=c("Variables","Residential", "Ambulatory", "p-value")
...
```{r 3_5_ser_pres, echo=T, error=T, paged.print=TRUE, eval=T}

  export2md(restab1_cont, size=11, first.strip=T, hide.no="no", position="center",
           format="html",caption= "Table. Summary descriptives at baseline, between Users that had contacts with the Judiciary System from 2010-2019 and those who did not")%>%#col.names=c("Variables","Residential", "Ambulatory", "p-value")
  kableExtra::add_footnote(c("Note. Continuous variables are presented as Medians and Percentiles 25 and 75 were shown;", "Categorical variables are presented as number (%)"), notation = "none")%>%
  kableExtra::scroll_box(width = "100%", height = "375px") %>% 
   kableExtra::kable_classic()
```
...
```{r 3_75_ser_pres, echo=T, error=T, paged.print=TRUE, eval=T}

  export2md(restab1_crim_rcds, size=11, first.strip=T, hide.no="no", position="center",
           format="html",caption= "Table. Summary descriptives at baseline, between Users with criminal records from PO data and those who did not")%>%#col.names=c("Variables","Residential", "Ambulatory", "p-value")
  kableExtra::add_footnote(c("Note. Continuous variables are presented as Medians and Percentiles 25 and 75 were shown;", "Categorical variables are presented as number (%)"), notation = "none")%>%
  kableExtra::scroll_box(width = "100%", height = "375px") %>% 
   kableExtra::kable_classic()
