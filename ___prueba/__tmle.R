if(!require(dplyr)){install.pacakges("dplyr")}
if(!require(tidyr)){install.pacakges("tidyr")}
if(!require(tmvtnorm)){install.pacakges("tmvtnorm")}
#if(!require(survdr)){install.pacakges("tmvtnorm")}
source("https://raw.githubusercontent.com/idiazst/survdr/master/R/functions.r")
source("https://raw.githubusercontent.com/idiazst/survdr/master/R/utils.r")
...
genData <- function(n, K, uni = TRUE){
  ...
  df <- df %>% group_by(id) %>%
    mutate(tmpR = cumsum(Rm), tmpL = cumsum(Lm),
           ...
           Jm = (tmpRR <= 1) * (tmpLL <= 1),
           Lm = Im * Lm, Rm = Jm * Rm) %>%
    dplyr::select(-tmpR, -tmpRR, -tmpL, -tmpLL)
