...
```{r exp-surv3-pris, eval=T, echo=F, error=T, fig.align='center', fig.cap="Cunulative Hazards of Offense (Difference with age of offense from baseline treatment outcome (Staggered entry)", fig.out="70%", message=FALSE, warning=FALSE, results='hide'}
library(cowplot)
library(gridExtra)
...
fortify(mot_egreso_fit2, fun = "cumhaz") %>% 
  ...
  ggtitle("Cond. Sent.")+ guides(color = guide_legend(ncol = 2),
                                 fill = guide_legend(ncol = 2))#,  legend.title=element_text(size=20), 
    #legend.text=element_text(size=18))
#theme(text = element_text(size=400))
...
fortify(mot_egreso_fit2_pris, fun = "cumhaz") %>% 
    ...
    annotate(geom = "table",
             ...
             table.theme = ttheme_gtminimal(base_size=15))+ #10 default
