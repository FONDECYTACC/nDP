...
fit0 <- coxph(Surv(time2=age_offending_imp, time=edad_al_egres_imp,  !is.na(dateofbirth_imp)) ~factor(motivodeegreso_mod_imp_rec), data=Base_fiscalia_v13, iter=0, na.action=na.exclude)
o.minus.e <- tapply(resid(fit0), Base_fiscalia_v13$motivodeegreso_mod_imp_rec, sum)
obs       <- tapply(!is.na(Base_fiscalia_v13$dateofbirth_imp), Base_fiscalia_v13$motivodeegreso_mod_imp_rec, sum)
db1 <- cbind.data.frame(observed=obs, expected= obs- o.minus.e, "o-e"=o.minus.e)
pchisq(((db1$`o-e`)[3]^2)/db1$expected[3],2, F)+ #3 categories = n-1
pchisq(((db1$`o-e`)[2]^2)/db1$expected[2],2, T) #3 categories = n-1
