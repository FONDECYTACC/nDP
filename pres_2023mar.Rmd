...
load(paste0(sub("$\\/","",sub("2019 \\(github\\)/SUD_CL","2022 \\(github\\)",dirname(rstudioapi::getSourceEditorContext()$path))),"/15.RData"))
...
•	Definir completar el tratamiento
•	Explicar las probabilidades de supervivencia condicional al tiempo
•	Mencionar HASH  y algoritmo de encriptación
•	Etnia y tiempo
•	Situación irregular (surrut) (¿Podrá existir personas con un RUT temporal que cambien posteriormente?)
•	Explicitar covariables elegidas y relaciones con T y O: historias, lógica e interpretación.
•	Justificar temporalidades o ventanas elegidas (por qué 3 y 6 meses, 1 y 3 años)
•	Cuidado con el lenguaje causal ("impacto" por "asociación")
•	Análisis de sesgos (ej., e-values)
...
tasks <- c("Revisión de la literatura", "Análisis estadístico principal", "Análisis de sensibilidad", "Reporte de progreso", "Arreglos finales", "Manuscrito publicable", "Presentación en congresos/seminarios")
tasks <- c("Literature review", "Main statistic analysis", "Sensitivity analyses", "Progress report", "Final arrangements", "Manuscript read for submission", "Presentation in scientific meetings")
...
dfr <- data.frame(
  ...
  start.date  = as.Date(c("2023-06-12", "2023-06-12", "2023-10-01", "2024-01-01", "2024-04-01","2024-04-01","2024-04-01")),
  end.date    = as.Date(c("2023-10-31", "2023-10-31", "2024-02-28", "2024-03-30","2024-06-30","2024-06-30","2024-06-30")),
  Critical = c("Yes", "No", "Yes", "Yes", "No", "Yes", "Yes"),
  Actor= factor(c("AGS, JRT & MM","AGS & JRT","AGS & JRT", "AGS, JRT & MM", "AGS, JRT & MM", "AGS, JRT & MM", "AGS, JRT & MM"))
)
for (i in 1:nrow(dfr)){
...
ggplot(dfr, aes(x =start.date, xend= end.date, y=factor(name, levels = rev(levels(name))), yend = factor(name, levels = rev(levels(name))))) +
  geom_segment(size = size_gplot*3.5,colour="gray30") +
  #scale_color_manual(values=c("gray70","gray30"))+# #FE4A17 ##21177A
  scale_color_manual(values="gray30")+# #FE4A17 ##21177A
  scale_fill_manual(values="gray30")+# #FE4A17 ##21177A
  xlab(NULL) + ylab(NULL)+
  theme_minimal()+
  geom_label(data=dfr, aes(x=mean_date, y=factor(name, levels = rev(levels(name))),  label = Actor), size=size_gplot*.6, color="gray40",show.legend=F#,
             #nudge_x = c(-300, -200, -200, -100)
             ) +
    scale_x_date(date_breaks = "2 months",date_labels="%b '%y",limits=c(as.Date("2023-06-01"),as.Date("2024-08-02")))+
  ...
  labs(caption="AGS= Andrés González; JRT= José Ruiz-Tagle; MM= Mariel Mateo")

#"800 x400" exportar

ggsave("_figs/gantt_mar2023.png",ggant, width = 8, height = 2, dpi = 400)
...
ggplot(dfr, aes(x =start.date, xend= end.date, y=factor(name, levels = rev(levels(name))), yend = factor(name, levels = rev(levels(name))), pattern_fill = Actor, label=Actor)) +
  ...
  geom_label(data=dfr, aes(x=mean_date, y=factor(name, levels = rev(levels(name))),  label = Actor), size=size_gplot2, color="gray20",show.legend=F#,
    ...
    scale_x_date(date_breaks = "2 months",date_labels="%b '%y",limits=c(as.Date("2023-06-01"),as.Date("2024-07-02")))+
