...
Several issues were found in the first stage of the exploration of the matches.
 ...
 - Investigate 0's in `IDSUJETO_VICTIMA`. ¿Have they the same distributed crimes? <!--- Done --->
 ...
 - Analyze these procedures "Anulación de ingreso error de digitación" & "Agrupado" (gls_mottermino)
...
as.data.frame.TableOne(CreateTableOne(vars= setdiff(c(myVars,"cut_com_del","cut_fec_nac","cut_com_del2","cut_fec_nac2"), c("gls_comuna", "gls_sitiosuceso", "gls_materia", "reg")), data= dplyr::mutate(Base_fiscalia_v5, sex_nodef=ifelse(sexo=="No Definido",1,0), cut_fec_nac=cut(imp_birth_date,10), cut_com_del=factor(cut_interval(fec_comision_simple,10)),cut_fec_nac2=cut2(imp_birth_date, cuts=as.Date(attr(binning(as.numeric(imp_birth_date)),"breaks"))),cut_com_del2=cut2(fec_comision_simple, cuts=as.Date(attr(binning(as.numeric(fec_comision_simple)),"breaks")))), factorVars = catVars, smd=T, strata="sex_nodef", addOverall = T, includeNA=T,test=T), smd=T)%>% 
...
```{r bpmn2-sexo, echo=T, cache= T, paged.print=TRUE, eval=T, error=T, fig.align="center", dev.args = list(bg = 'white')}
#Ñ#Ñ#Ñ#Ñ#Ñ#Ñ#Ñ#Ñ#Ñ
#Ñ#Ñ#Ñ#Ñ#Ñ#Ñ#Ñ#Ñ#Ñ++++++▼
...
There are `r Base_fiscalia_v6%>% dplyr::left_join(dplyr::left_join(differences_bet_sex_po3[c("rut_enc_saf","flowch_sex")], CONS_C1_tr_sex, by=c("rut_enc_saf"="HASH_KEY")), by="rut_enc_saf") %>%  dplyr::filter(length_women_tr>0, grepl("22_2_a1f",flowch_sex))%>%  nrow() %>%  format(big.mark=",")` records (individuals= `r Base_fiscalia_v6%>% dplyr::left_join(dplyr::left_join(differences_bet_sex_po3[c("rut_enc_saf","flowch_sex")],CONS_C1_tr_sex, by=c("rut_enc_saf"="HASH_KEY")), by="rut_enc_saf") %>%dplyr::filter(length_women_tr>0,grepl("22_2_a1f",flowch_sex))%>%  dplyr::group_by(rut_enc_saf) %>% distinct(rut_enc_saf) %>% nrow() %>% format(big.mark=",")`) corresponding to men receiving women-only treatments. **These should be replaced once the merging is done**.
...
paste0("Cases with discrepancies in PO & SENDA that had");paste0("more than one value in nationality different than CHILE= ",
CONS_C1_res_nat %>% 
    rowwise %>%
...
```{r bpmn2-nat, echo=T, cache= T, paged.print=TRUE, eval=T, error=T, fig.align="center", dev.args = list(bg = 'white')}
bpmn_file <- system.file(paste0(sub("2019 \\(github\\)/SUD_CL","2022 \\(github\\)",path), "/_figs/diagram_sex_differences.bpmn"), package = "bpmn")
...
CreateTableOne(vars= setdiff(c(myVars,"cut_com_del2","cut_fec_nac2"), c("gls_comuna", "gls_sitiosuceso","tipo_sujeto_vic ", "reg")), data=  dplyr::mutate(Base_fiscalia_v7, idsujeto_victima_0=ifelse(idsujeto_victima==0,1,0),cut_fec_nac2=cut2(imp_birth_date, cuts=as.Date(attr(binning(as.numeric(imp_birth_date)),"breaks"))),cut_com_del2=cut2(fec_comision_simple, cuts=as.Date(attr(binning(as.numeric(fec_comision_simple)),"breaks")))) %>% data.table(), factorVars = setdiff(catVars, c("gls_comuna", "gls_sitiosuceso", "reg")), smd=T, strata="idsujeto_victima_0", addOverall = T, includeNA=T,test=T)
...
Registries with no ID are far more frequent in the north: I Región de Tarapaca, II Región de Antofagasta and XV Región de Arica y Parinacota, while less in the Metropolitan Region. Also have judiciary finishings of the proceedings. Much more were applied a "PRINCIPIO DE OPORTUNIDAD" and "SENTENCIA DEFINITIVA CONDENATORIA", and less ended in  "ARCHIVO PROVISIONAL", "AGRUPACI¿N A OTRO CASO" and "SOBRESEIMIENTO DEFINITIVO". Many of the crimes were related to drunk driving, posession of drugs and knives or sharp objects, brawls, drug traffic, microtraffic. There are no cases related to domestic violence. The order for payment procedure ("MONITORIO") was more frequent, in contrast to the ordinary procedure ("ORDINARIO"). The place in which the crime was commited more frequently in national or municipal goods of public use ("BIENES NACIONALES USO PUBLICO"), but less in residential places ("LUGAR HABITADO"). Also, these crimes were more frequently commited between 2010 and 2015, but less after 2015.

<br>

```{r 2-exp-miss-val, echo=T, fig.align='center', message=T, error=T, eval=T}
tbone_tipo_termino_na<-
CreateTableOne(vars= setdiff(c(myVars,"cut_com_del2","cut_fec_nac2"), c("gls_comuna", "gls_sitiosuceso", "tipo_sujeto_vic ", "reg", "gls_materia")), data=  dplyr::mutate(Base_fiscalia_v7, tipo_termino_NA=ifelse(is.na(tipo_termino),1,0),cut_fec_nac2=cut2(imp_birth_date, cuts=as.Date(attr(binning(as.numeric(imp_birth_date)),"breaks"))),cut_com_del2=cut2(fec_comision_simple, cuts=as.Date(attr(binning(as.numeric(fec_comision_simple)),"breaks")))) %>% data.table(), factorVars = setdiff(catVars, c("gls_comuna", "gls_sitiosuceso", "reg")), smd=T, strata="tipo_termino_NA", addOverall = T, includeNA=T,test=T)
...
CreateTableOne(vars= setdiff(c(myVars,"cut_com_del2","cut_fec_nac2"), c("gls_comuna", "gls_sitiosuceso", "tipo_sujeto_vic ", "reg", "gls_mottermino", "gls_materia")), data=  dplyr::mutate(Base_fiscalia_v7, mot_termino_NA=ifelse(is.na(gls_mottermino),1,0),cut_fec_nac2=cut2(imp_birth_date, cuts=as.Date(attr(binning(as.numeric(imp_birth_date)),"breaks"))),cut_com_del2=cut2(fec_comision_simple, cuts=as.Date(attr(binning(as.numeric(fec_comision_simple)),"breaks")))) %>% data.table(), factorVars = setdiff(catVars, c("gls_comuna", "gls_sitiosuceso", "reg", "tipo_termino")), smd=T, strata="mot_termino_NA", addOverall = T, includeNA=T,test=T)
...
as.data.frame.TableOne(tbone_mot_termino_na, smd=T)%>% 
  ...
  dplyr::mutate(level=ifelse(is.na(level),"[Missing]",level)) %>% 
  dplyr::mutate(char2=dplyr::case_when(characteristic=="NA"~NA_character_,T~as.character(characteristic))) %>% 
kable(size=11, format="html",caption= "Table. Summary descriptives, Missing values in the Cause of termination of the relationship(1)",smd=T, test=T, varLabels=T,noSpaces=T, printToggle=T, dropEqual=F)%>%
  kableExtra::kable_classic(bootstrap_options = c("striped", "hover","condensed"),font_size= 10) %>%
  row_spec(1, bold=T, italic=T,color="black",hline_after=T,extra_latex_after="\\arrayrulecolor{white}",font_size= 11) %>%
  kableExtra::add_footnote(c("Note. Continuous variables are presented as Medians and Percentiles 25 and 75 were shown;", "Categorical variables are presented as number (%)"), notation = "none")%>%
  kableExtra::scroll_box(width = "100%", height = "375px")
...
CreateTableOne(vars= setdiff(c(myVars,"cut_com_del2","cut_fec_nac2"), c("gls_comuna", "gls_sitiosuceso", "tipo_sujeto_vic ", "reg", "gls_mottermino", "gls_materia")), data=  dplyr::mutate(Base_fiscalia_v7, agrupa_terminos_anul=ifelse(agrupa_terminos=="ANULACI¿N ADMINISTRATIVA",1,0),cut_fec_nac2=cut2(imp_birth_date, cuts=as.Date(attr(binning(as.numeric(imp_birth_date)),"breaks"))),cut_com_del2=cut2(fec_comision_simple, cuts=as.Date(attr(binning(as.numeric(fec_comision_simple)),"breaks")))) %>% data.table(), factorVars = setdiff(catVars, c("gls_comuna", "gls_sitiosuceso", "reg", "tipo_termino")), smd=T, strata="agrupa_terminos_anul", addOverall = T, includeNA=T,test=T)
...
as.data.frame.TableOne(tbone_agrup_ter_anul, smd=T)%>% 
  ...
  dplyr::mutate(level=ifelse(is.na(level),"[Missing]",level)) %>% 
  dplyr::mutate(char2=dplyr::case_when(characteristic=="NA"~NA_character_,T~as.character(characteristic))) %>% 
kable(size=11, format="html",caption= "Table. Summary descriptives, Values in the Cause of termination of the relationship, Administrative annulment(1)",smd=T, test=T, varLabels=T,noSpaces=T, printToggle=T, dropEqual=F)%>%
  kableExtra::kable_classic(bootstrap_options = c("striped", "hover","condensed"),font_size= 10) %>%
  row_spec(1, bold=T, italic=T,color="black",hline_after=T,extra_latex_after="\\arrayrulecolor{white}",font_size= 11) %>%
  kableExtra::add_footnote(c("Note. Continuous variables are presented as Medians and Percentiles 25 and 75 were shown;", "Categorical variables are presented as number (%)"), notation = "none")%>%
  kableExtra::scroll_box(width = "100%", height = "375px")
...
```{r 4-5-exp-miss-val, echo=T, fig.align='center', message=T, error=T, eval=T}
#- `TIPO_SUJETO_VIC` that are victims and imputed at the same time

tbone_suj_vic_imp<-
CreateTableOne(vars= setdiff(c(myVars,"cut_com_del2","cut_fec_nac2"), c("gls_comuna", "gls_sitiosuceso", "tipo_sujeto_vic", "reg", "gls_mottermino", "gls_materia")), data=  dplyr::mutate(Base_fiscalia_v7, agrupa_terminos_anul=ifelse(encontrado_como_victima=="SI"& encontrado_como_imputado=="SI",1,0),cut_fec_nac2=cut2(imp_birth_date, cuts=as.Date(attr(binning(as.numeric(imp_birth_date)),"breaks"))),cut_com_del2=cut2(fec_comision_simple, cuts=as.Date(attr(binning(as.numeric(fec_comision_simple)),"breaks")))) %>% data.table(), factorVars = setdiff(catVars, c("gls_comuna", "gls_sitiosuceso", "reg", "tipo_termino")), smd=T, strata="agrupa_terminos_anul", addOverall = T, includeNA=T,test=T)
...
as.data.frame.TableOne(tbone_suj_vic_imp, smd=T)%>% 
  ...
  dplyr::mutate(level=ifelse(is.na(level),"[Missing]",level)) %>% 
  dplyr::mutate(char2=dplyr::case_when(characteristic=="NA"~NA_character_,T~as.character(characteristic))) %>% 
kable(size=11, format="html",caption= "Table. Summary descriptives, Relationships of people both victim and accused",smd=T, test=T, varLabels=T,noSpaces=T, printToggle=T, dropEqual=F)%>%
  kableExtra::kable_classic(bootstrap_options = c("striped", "hover","condensed"),font_size= 10) %>%
  row_spec(1, bold=T, italic=T,color="black",hline_after=T,extra_latex_after="\\arrayrulecolor{white}",font_size= 11) %>%
  kableExtra::add_footnote(c("Note. Continuous variables are presented as Medians and Percentiles 25 and 75 were shown;", "Categorical variables are presented as number (%)"), notation = "none")%>%
  kableExtra::scroll_box(width = "100%", height = "375px")
...
`COD_DELITO` with value 0 is "Not defined"

```{r 6-exp-crimes, echo=T, fig.align='center', message=T, error=T, eval=T, fig.align='center', fig.pos='H', fig.cap="Not defined ", dpi=320}
crime_cod_label<-
...
crimes_no_cases<-
Base_fiscalia_v7 %>% 
  dplyr::mutate(cut_fec_nac2=cut2(imp_birth_date, cuts=as.Date(attr(binning(as.numeric(imp_birth_date)),"breaks"))),cut_com_del2=cut2(fec_comision_simple, cuts=as.Date(attr(binning(as.numeric(fec_comision_simple)),"breaks")))) %>% data.table() %>% 
...
Base_fiscalia_v7 %>% 
  ...
  ungroup()%>%
  dplyr::filter(variable %in% as.numeric(unlist(crimes_no_cases)))%>%
  knitr::kable("html",caption= "Crimes that had a null count in one time window", col.names=c("Dates","Crime (Code)", "Count","Details"))%>% 
...
```{r 7-iddelito-cod-delito, echo=T, fig.align='center', message=T, error=T, eval=T}
#- Explore differences of `IDDELITO` with `cod_delito`.
Base_fiscalia_v7 %>% 
  dplyr::group_by(cod_delito,iddelito) %>% 
  summarise(n=n()) %>% 
  head(20) %>% 
  knitr::kable("html",caption= "ID of crimes in each causae, by the code of crime (First 20 cases)", col.names=c("Crime code","Crime ID", "Count"))%>% 
  kableExtra::kable_classic()%>%
  kableExtra::scroll_box(width = "100%", height = "375px")

message(paste0("More than one distinct crime codes by ID of crime= ",
Base_fiscalia_v7 %>% 
...
The lowest 5% of the dates of crimes committed were in `r as.Date(quantile(unclass(Base_fiscalia_v7$fec_comision_simple), .05))` or older. We decided to explore these entries.

<br>

```{r 2-date-comission-perc-05, echo=T, fig.align='center', message=T, error=T, eval=T}
tbone_date_comission_perc_05<-
CreateTableOne(vars= setdiff(c(myVars,"cut_com_del2","cut_fec_nac2"), c("gls_comuna", "gls_sitiosuceso", "tipo_sujeto_vic", "reg", "gls_mottermino","gls_materia")), data=  dplyr::mutate(Base_fiscalia_v7, date_comission_perc_05=ifelse(fec_comision_simple<as.Date(quantile(unclass(Base_fiscalia_v7$fec_comision_simple), .05)),1,0),cut_fec_nac2=cut2(imp_birth_date, cuts=as.Date(attr(binning(as.numeric(imp_birth_date)),"breaks")))) %>% data.table(), factorVars = setdiff(catVars, c("gls_comuna", "gls_sitiosuceso", "reg", "tipo_termino")), smd=T, strata="date_comission_perc_05", addOverall = T, includeNA=T,test=T)
...
as.data.frame.TableOne(tbone_date_comission_perc_05, smd=T)%>% 
  ...
  dplyr::mutate(level=ifelse(is.na(level),"[Missing]",level)) %>% 
  dplyr::mutate(char2=dplyr::case_when(characteristic=="NA"~NA_character_,T~as.character(characteristic))) %>% 
kable(size=11, format="html",caption= paste0("Table. Summary descriptives, Date comission before ",unlist(as.character(as.Date(quantile(unclass(Base_fiscalia_v7$fec_comision_simple), .05))))),smd=T, test=T, varLabels=T,noSpaces=T, printToggle=T, dropEqual=F)%>%
  kableExtra::kable_classic(bootstrap_options = c("striped", "hover","condensed"),font_size= 10) %>%
  row_spec(1, bold=T, italic=T,color="black",hline_after=T,extra_latex_after="\\arrayrulecolor{white}",font_size= 11) %>%
  kableExtra::add_footnote(c("Note. Continuous variables are presented as Medians and Percentiles 25 and 75 were shown;", "Categorical variables are presented as number (%)"), notation = "none")%>%
  kableExtra::scroll_box(width = "100%", height = "375px")
...
#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_
```
```{r 2b-date-crimecomission, echo=T, fig.align='center', message=T, error=T, eval=T}
Base_fiscalia_v7 %>%
  dplyr::mutate(obs_age_com_crime= factor(dplyr::case_when(fec_comision_simple <"2010-01-01"~"a. Less 2010",edad_ter_rel_imp < edad_comision_imp~ "b. Age of termination of relationship < Age relationship", edad_comision_imp<0~ "c. Less 0 years age crime committed", edad_comision_imp < 18~ "d. Less 18 years age crime committed", is.na(edad_comision_imp)~"e. Missing age of crime comission", is.na(fec_comision_simple)~"f. Missing of date of crime comission", is.na(imp_birth_date)~ "g. MIssing birth date", T~"0. none"), levels=c("a. Less 2010", "b. Age of termination of relationship < Age relationship", "c. Less 0 years age crime committed", "d. Less 18 years age crime committed", "e. Missing age of crime comission", "f. Missing of date of crime comission", "g. MIssing birth date", "0. none"))) %>% 
...
dplyr::select(rut_enc_saf, encontrado_como_imputado, fec_comision_simple, fec_cbiorelacion_simple, edad_ter_rel_imp, edad_comision_imp, imp_birth_date, gls_region, gls_materia, agrupa_terminos, obs) %>% 
  ...
  head(20) %>% 
  knitr::kable(format="html",caption= "Cases w/ crimes committed before 2010 (First 20 cases)") %>% 
  kableExtra::kable_classic()

message(paste0("Time HASHs repeat, Cases w/ relationship date finished before the crime was committed"))
Base_fiscalia_v7 %>%
    dplyr::filter(rut_enc_saf %in% unlist(distinct(dplyr::filter(Base_fiscalia_v7,fec_comision_simple<"2010-01-01"),rut_enc_saf))) %>% 
    group_by(rut_enc_saf) %>% 
    dplyr::summarise(n=n()) %>% 
    dplyr::arrange(-n) %>% 
    #janitor::tabyl(n) %>% 
    #dplyr::arrange(n_n) %>% 
    mutate(bin = ntile(n, 8)) %>% 
    dplyr::group_by(bin) %>% 
    dplyr::mutate(sum_n=sum(n)) %>% 
    dplyr::summarise(records_per_hash=paste0(min(n)," - ", max(n)), records=first(sum_n))


invisible("Observaciones a casos anómalos 5%")
...
Base_fiscalia_v7 %>%
  ...
  head(20) %>% 
  knitr::kable(format="html",caption= "Cases w/ Age of finishing of relationship before the crimes being committed (First 20 cases)") %>% 
  kableExtra::kable_classic()%>%
  kableExtra::scroll_box(width = "100%", height = "375px")
...
message(paste0("Time HASHs repeat, Cases w/ relationship date finished before the crime was committed"))
Base_fiscalia_v7 %>%
    dplyr::filter(rut_enc_saf %in% unlist(distinct(dplyr::filter(Base_fiscalia_v7,edad_ter_rel_imp < edad_comision_imp),rut_enc_saf))) %>% 
    group_by(rut_enc_saf) %>% 
    dplyr::summarise(n=n()) %>% 
    dplyr::arrange(-n) %>% 
    #janitor::tabyl(n) %>% 
