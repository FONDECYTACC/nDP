...
## Analytical Plan
The study controlled for various confounding variables listed in Figure 1. Patients were weighted by the inverse probability of PSU (IPWs) based on several predictors. Weights were truncated at the 1st and 99th percentiles$`r Cite(myBib, c("Cole2008"))`$.
...
```
We used the illness-death multistate structure to estimate transitions from admission to treatment outcome, treatment outcome to contact with CJS, and admission to contact with CJS (without completing treatment) for patients with PSU and no PSU. 
...
```
We calculated the Aalen-Johansen estimator for transition probabilities at 6 months, 1 & 3 years using `multistate` in Stata$`r Cite(myBib, c("Crowther2023"))`$. Secondary analyses will focus on mediating effects of treatment outcome and using a time-to-first-event approach$`r Cite(myBib, c("VanderWeele2011a","s457128", "hill2022development"))`$. Markdowns & codes are available on https://fondecytacc.github.io/nDP/.

<!---
#To do so, we describe the cumulative incidence rate and incidence rate ratio (IRR) of contact with the criminal justice system (any contact; and contact leading to imprisonment after baseline treatment) and its variation by baseline SUT completion status. 
--->
...
irr_cont_js_wgt$value<- stringr::str_replace_all(irr_cont_js_wgt$value, pattern = "\\b(\\d+\\.\\d{2})\\d+\\b", function(x) round(as.numeric(x), ))
