...
```{r bring-db1-Fiscalia-merge5, echo=T, fig.align='center', message=T, error=T, eval=T}
...
```
```{r bring-db2-Fiscalia-merge5, echo=T, fig.align='center', message=T, error=T, eval=T}
Base_fiscalia_v14<-
Base_fiscalia_v13 %>% 
...
Base_fiscalia_v14_pris$clas_r <- relevel(factor(Base_fiscalia_v14_pris$Clasificación), ref = "Urbana")
...
Base_fiscalia_v14_filt%>% 
    dplyr::select(setdiff(c(vars_cov,"offender_d"), c("edad_al_ing_1", "edad_ini_cons","ano_nac_corr", "porc_pobr"))) %>% 
    dplyr::mutate(offender_d= ifelse(!is.na(offender_d),1,0)) %>% 
...
#chisq.test(table(Base_fiscalia_v14_filt$motivodeegreso_mod_imp_rec, Base_fiscalia_v14_filt$sus_principal_mod, exclude=NULL))
    ...
    rio::export("tableone_grant_22_23_3_alt_final.xlsx")

Base_fiscalia_v14_pris_filt%>% 
    dplyr::select(setdiff(c(vars_cov,"offender_d"), c("edad_al_ing_1", "edad_ini_cons","ano_nac_corr", "porc_pobr"))) %>% 
    dplyr::mutate(offender_d= ifelse(!is.na(offender_d),1,0)) %>% 
    tidyr::gather(variable,measure, -motivodeegreso_mod_imp_rec) %>% 
    dplyr::group_by(variable) %>%
...
#chisq.test(table(Base_fiscalia_v14_filt$motivodeegreso_mod_imp_rec, Base_fiscalia_v14_filt$sus_principal_mod, exclude=NULL))
    ...
    dplyr::mutate(report=paste0("X²(",parameter,", ",x,")=",statistic,"; p", ifelse(p.value=="<0.001",p.value, paste0("=",p.value)))) %>%
    dplyr::mutate(report=sub("0\\.","0,",sub("\\.",",",report)))%>%
    rio::export("tableone_grant_22_23_4.xlsx")

 chisq.test(Base_fiscalia_v14_filt$motivodeegreso_mod_imp_rec, is.na(Base_fiscalia_v14_filt$offender_d)) 
 sum(table(Base_fiscalia_v14_filt$motivodeegreso_mod_imp_rec, Base_fiscalia_v14_filt$offender_d))
...
```{r exp-surv23-1-supp, eval=T, echo=F, error=T, fig.align='center', message=FALSE, warning=FALSE}
prob_imp_diff_alt2<-rvest::read_html("prob_condsent_m2_main_diff.html") %>% rvest::html_table()
rmst_imp_diff_alt2<-rvest::read_html("rmst_condsent_m2_main_diff.html") %>% rvest::html_table()

prob_imp_diff_cc_tab<-
prob_imp_diff_cc %>% as.data.table() %>% janitor::row_to_names(3) %>% data.frame() %>% 
  dplyr::mutate_at(2:ncol(.), ~sprintf("%1.1f",readr::parse_number(.))) %>% 
  dplyr::rename("Time" = 1) %>% mutate(Comp_Early_ci= paste0(Comp_Early," (", Comp_Early_lci,",",Comp_Early_uci,")"), Comp_Late_ci = paste0(Comp_Late," (", Comp_Late_lci,",",Comp_Late_uci,")"), Early_Late_ci = paste0(Early_Late," (", Early_Late_lci,",",Early_Late_uci,")")) %>% 
  dplyr::select(Time, Comp_Late_ci, Comp_Early_ci, Early_Late_ci) %>% 
  dplyr::filter(!Time %in% c("3_mths","6_mths")) %>% 
  dplyr::filter(!Time %in% c("3_mths","6_mths"))

rmst_imp_diff_cc_tab<-
rmst_imp_diff_cc %>% as.data.table() %>% janitor::row_to_names(3) %>% data.frame() %>% 
  dplyr::mutate_at(2:ncol(.), ~sprintf("%1.3f",readr::parse_number(.))) %>% 
  dplyr::rename("Time" = 1) %>% mutate(Comp_Early_ci= paste0(Comp_Early," (", Comp_Early_lci,",",Comp_Early_uci,")"), Comp_Late_ci = paste0(Comp_Late," (", Comp_Late_lci,",",Comp_Late_uci,")"), Early_Late_ci = paste0(Early_Late," (", Early_Late_lci,",",Early_Late_uci,")")) %>% 
  dplyr::select(Time, Comp_Late_ci, Comp_Early_ci, Early_Late_ci) %>% 
...
prob_imp_diff_alt2_tab<-
prob_imp_diff_alt2 %>% as.data.table() %>% janitor::row_to_names(3) %>% data.frame() %>% 
  dplyr::mutate_at(2:ncol(.), ~sprintf("%1.1f",readr::parse_number(.))) %>% 
  dplyr::rename("Time" = 1) %>% mutate(Comp_Early_ci= paste0(Comp_Early," (", Comp_Early_lci,",",Comp_Early_uci,")"), Comp_Late_ci = paste0(Comp_Late," (", Comp_Late_lci,",",Comp_Late_uci,")"), Early_Late_ci = paste0(Early_Late," (", Early_Late_lci,",",Early_Late_uci,")")) %>% 
  dplyr::select(Time, Comp_Late_ci, Comp_Early_ci, Early_Late_ci) %>% 
  dplyr::filter(!Time %in% c("3_mths","6_mths")) %>% 
...
rmst_imp_diff_alt2_tab<-
rmst_imp_diff_alt2 %>% as.data.table() %>% janitor::row_to_names(3) %>% data.frame() %>% 
  dplyr::mutate_at(2:ncol(.), ~sprintf("%1.3f",readr::parse_number(.))) %>% 
  dplyr::rename("Time" = 1) %>% mutate(Comp_Early_ci= paste0(Comp_Early," (", Comp_Early_lci,",",Comp_Early_uci,")"), Comp_Late_ci = paste0(Comp_Late," (", Comp_Late_lci,",",Comp_Late_uci,")"), Early_Late_ci = paste0(Early_Late," (", Early_Late_lci,",",Early_Late_uci,")")) %>% 
  dplyr::select(Time, Comp_Late_ci, Comp_Early_ci, Early_Late_ci) %>% 
  dplyr::filter(!Time %in% c("3_mths","6_mths")) 
...
rbindlist(list(list(c("Probs.",rep("",0))),cbind.data.frame(prob_imp_diff_cc_tab, prob_imp_diff_alt2_tab[,-1]),
               list(c("RMST",rep("",0))), cbind.data.frame(rmst_imp_diff_cc_tab, rmst_imp_diff_alt2_tab[,-1])), fill=T) %>% janitor::clean_names() %>% 
    #dplyr::slice(-2,-3,-4,-5,-6,-7,-14,-15,-16,-17,-18,-19) %>%        
  dplyr::filter(nchar(time)>=1) %>% 
  #knitr::kable("markdown", size=3, col.names= c("Time", "Complete Tr.","Late Disch.", "Early Disch.", "Comp. vs Late", "Comp. vs Early", "Early vs Late"), caption= "Offending with Condemnatory Sentence")
kbl(format = 'markdown',
        escape = FALSE,
        col.names= c("Time", "Comp. vs Late", "Comp. vs Early", "Early vs Late", "Comp. vs Late", "Comp. vs Early", "Early vs Late"), 
    caption= "Offending with Condemnatory Sentence")
```


```{r exp-surv23-2-supp, eval=T, echo=F, error=T, fig.align='center', message=FALSE, warning=FALSE}
prob_imp_diff_pris_alt2<-rvest::read_html("prob_prison_m2_main_diff.html") %>% rvest::html_table()
rmst_imp_diff_pris_alt2<-rvest::read_html("rmst_prison_m2_main_diff.html") %>% rvest::html_table()


prob_imp_diff_pris_cc_tab<-
prob_imp_diff_pris_cc %>% as.data.table() %>% janitor::row_to_names(3) %>% data.frame() %>% 
  dplyr::mutate_at(2:ncol(.), ~sprintf("%1.1f",readr::parse_number(.))) %>% 
  dplyr::rename("Time" = 1) %>% mutate(Comp_Early_ci= paste0(Comp_Early," (", Comp_Early_lci,",",Comp_Early_uci,")"), Comp_Late_ci = paste0(Comp_Late," (", Comp_Late_lci,",",Comp_Late_uci,")"), Early_Late_ci = paste0(Early_Late," (", Early_Late_lci,",",Early_Late_uci,")")) %>% 
  dplyr::select(Time, Comp_Late_ci, Comp_Early_ci, Early_Late_ci) %>% 
...
prob_imp_diff_pris_alt2_tab<-
prob_imp_diff_pris_alt2 %>% as.data.table() %>% janitor::row_to_names(3) %>% data.frame() %>% 
  dplyr::mutate_at(2:ncol(.), ~sprintf("%1.1f",readr::parse_number(.))) %>% 
  dplyr::rename("Time" = 1) %>% mutate(Comp_Early_ci= paste0(Comp_Early," (", Comp_Early_lci,",",Comp_Early_uci,")"), Comp_Late_ci = paste0(Comp_Late," (", Comp_Late_lci,",",Comp_Late_uci,")"), Early_Late_ci = paste0(Early_Late," (", Early_Late_lci,",",Early_Late_uci,")")) %>% 
  dplyr::select(Time, Comp_Late_ci, Comp_Early_ci, Early_Late_ci) %>% 
  dplyr::filter(!Time %in% c("3_mths","6_mths")) %>% 
...
rmst_imp_diff_pris_alt2_tab<-
rmst_imp_diff_pris_alt2 %>% as.data.table() %>% janitor::row_to_names(3) %>% data.frame() %>% 
  dplyr::mutate_at(2:ncol(.), ~sprintf("%1.3f",readr::parse_number(.))) %>% 
  dplyr::rename("Time" = 1) %>% mutate(Comp_Early_ci= paste0(Comp_Early," (", Comp_Early_lci,",",Comp_Early_uci,")"), Comp_Late_ci = paste0(Comp_Late," (", Comp_Late_lci,",",Comp_Late_uci,")"), Early_Late_ci = paste0(Early_Late," (", Early_Late_lci,",",Early_Late_uci,")")) %>% 
  dplyr::select(Time, Comp_Late_ci, Comp_Early_ci, Early_Late_ci) %>% 
  dplyr::filter(!Time %in% c("3_mths","6_mths")) 
...
rbindlist(list(list(c("Probs.",rep("",0))),cbind.data.frame(prob_imp_diff_pris_cc_tab, prob_imp_diff_pris_alt2_tab[,-1]),
               list(c("RMST",rep("",0))), cbind.data.frame(rmst_imp_diff_pris_cc_tab, rmst_imp_diff_pris_alt2_tab[,-1])), fill=T) %>% janitor::clean_names() %>% 
    #dplyr::slice(-2,-3,-4,-5,-6,-7,-14,-15,-16,-17,-18,-19) %>%        
  dplyr::filter(nchar(time)>=1) %>% 
  #knitr::kable("markdown", size=3, col.names= c("Time", "Complete Tr.","Late Disch.", "Early Disch.", "Comp. vs Late", "Comp. vs Early", "Early vs Late"), caption= "Offending with Condemnatory Sentence")
kbl(format = 'markdown',
