...
fmm <- cph(Surv(edad_al_egres_imp,age_offending_imp,  !is.na(dateofbirth_imp)) ~ motivodeegreso_mod_imp_rec + edad_al_ing_1 + edad_ini_cons + sex+ escolaridad_rec + sus_principal_mod + freq_cons_sus_prin + compromiso_biopsicosocial + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + policonsumo + n_off_vio + n_off_acq + n_off_sud, data= Base_fiscalia_v13, x=T, y=T)
...
x<-
c("motivodeegreso_mod_imp_rec", "edad_al_ing_1", "edad_ini_cons", "sex", "escolaridad_rec", "sus_principal_mod", "freq_cons_sus_prin", "compromiso_biopsicosocial", "origen_ingreso_mod", "numero_de_hijos_mod", "tenencia_de_la_vivienda_mod", "dg_cie_10_rec", "dg_trs_cons_sus_or", "macrozona", "policonsumo", "n_off_vio", "n_off_acq", "n_off_sud")
...
```{r miss, warning=FALSE, echo=T, error=T, eval=T}
#https://www.datacamp.com/tutorial/ensemble-r-machine-learning

set.seed(2125)
system.time({
model <- SuperLearner(Y = y, X = x, family = binomial(), SL.library=list("SL.ranger",
                                          "SL.ksvm",
                                          "SL.ipredbagg",
                                          "SL.bayesglm"))
})
...
cm <- caret::confusionMatrix(conv.preds, ytest)
```
