...
Base_fiscalia_v13 %>% 
  ...
  dplyr::mutate(across(c("motivodeegreso_mod_imp_rec","sus_principal_mod", "origen_ingreso_mod", "tenencia_de_la_vivienda_mod", "condicion_ocupacional_corr", "dg_cie_10_rec", "macrozona", "n_off_vio", "n_off_acq", "n_off_sud", "classification"),~as.factor(.)))%>% 
...
survRM2::rmst2(yrs_to_condemn, event_r, tr_comp_comp_cases, tau=.33)

cat("Treatment completion vs. Late and Early Dropouts at 1 year")
survRM2::rmst2(yrs_to_condemn, event_r, tr_comp_comp_cases, tau=1)

cat("Treatment completion vs. Late and Early Dropouts at 3 years")
survRM2::rmst2(yrs_to_condemn, event_r, tr_comp_comp_cases, tau=3)

cat("Treatment completion vs. Late and Early Dropouts at 5 years")
survRM2::rmst2(yrs_to_condemn, event_r, tr_comp_comp_cases, tau=5)
...
coxfit_more_var1 <- coxph(Surv(yrs_to_condemn,  event_r) ~ factor(motivodeegreso_mod_imp_rec) + edad_al_ing_1 + edad_ini_cons + dias_treat_imp_sin_na_1 + factor(escolaridad_rec) + factor(sus_principal_mod) + freq_cons_sus_prin + condicion_ocupacional_corr+ via_adm_sus_prin_act+ origen_ingreso_mod + numero_de_hijos_mod + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + n_prev_off + n_off_vio + n_off_acq + n_off_sud + classification+ clas_r + porc_pobr, ties="efron", data= Base_fiscalia_v14)#[1:250,]

coxfit_more_var2 <- coxph(Surv(yrs_to_condemn,  event_r) ~ motivodeegreso_mod_imp_rec + rcs(edad_al_ing_1,4) + edad_ini_cons + dias_treat_imp_sin_na_1 + escolaridad_rec + sus_principal_mod + freq_cons_sus_prin + condicion_ocupacional_corr+ via_adm_sus_prin_act+ origen_ingreso_mod + numero_de_hijos_mod + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + n_prev_off + n_off_vio + n_off_acq + n_off_sud + classification+ clas_r + porc_pobr, ties="efron", data= Base_fiscalia_v14)#[1:250,]

coxfit_more_var3 <- coxph(Surv(yrs_to_condemn,  event_r) ~ motivodeegreso_mod_imp_rec + rcs(edad_al_ing_1,3) + edad_ini_cons + dias_treat_imp_sin_na_1 + escolaridad_rec + sus_principal_mod + freq_cons_sus_prin + condicion_ocupacional_corr+ via_adm_sus_prin_act+ origen_ingreso_mod + numero_de_hijos_mod + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + n_prev_off + n_off_vio + n_off_acq + n_off_sud + classification+ clas_r + porc_pobr, ties="efron", data= Base_fiscalia_v14)#[1:250,]

coxfit_more_var4 <- coxph(Surv(yrs_to_condemn,  event_r) ~ motivodeegreso_mod_imp_rec + rcs(edad_al_ing_1,5) + edad_ini_cons + dias_treat_imp_sin_na_1 + escolaridad_rec + sus_principal_mod + freq_cons_sus_prin + condicion_ocupacional_corr+ via_adm_sus_prin_act+ origen_ingreso_mod + numero_de_hijos_mod + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + n_prev_off + n_off_vio + n_off_acq + n_off_sud + classification+ clas_r + porc_pobr, ties="efron", data= Base_fiscalia_v14)#[1:250,]

coxfit_more_var5 <- coxph(Surv(yrs_to_condemn,  event_r) ~ motivodeegreso_mod_imp_rec + rcs(edad_al_ing_1,6) + edad_ini_cons + dias_treat_imp_sin_na_1 + escolaridad_rec + sus_principal_mod + freq_cons_sus_prin + condicion_ocupacional_corr+ via_adm_sus_prin_act+ origen_ingreso_mod + numero_de_hijos_mod + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + n_prev_off + n_off_vio + n_off_acq + n_off_sud + classification+ clas_r + porc_pobr, ties="efron", data= Base_fiscalia_v14)#[1:250,]

coxfit_more_var6 <- coxph(Surv(yrs_to_condemn,  event_r) ~ motivodeegreso_mod_imp_rec + ns(edad_al_ing_1) + edad_ini_cons + dias_treat_imp_sin_na_1 + escolaridad_rec + sus_principal_mod + freq_cons_sus_prin + condicion_ocupacional_corr+ via_adm_sus_prin_act+ origen_ingreso_mod + numero_de_hijos_mod + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + n_prev_off + n_off_vio + n_off_acq + n_off_sud + classification+ clas_r + porc_pobr, ties="efron", data= Base_fiscalia_v14)#[1:250,]

coxfit_more_var7 <- coxph(Surv(yrs_to_condemn,  event_r) ~ motivodeegreso_mod_imp_rec + ns(edad_al_ing_1, df=2) + edad_ini_cons + dias_treat_imp_sin_na_1 + escolaridad_rec + sus_principal_mod + freq_cons_sus_prin + condicion_ocupacional_corr+ via_adm_sus_prin_act+ origen_ingreso_mod + numero_de_hijos_mod + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + n_prev_off + n_off_vio + n_off_acq + n_off_sud + classification+ clas_r + porc_pobr, ties="efron", data= Base_fiscalia_v14)#[1:250,]

coxfit_more_var8 <- coxph(Surv(yrs_to_condemn,  event_r) ~ motivodeegreso_mod_imp_rec + ns(edad_al_ing_1, df=3) + edad_ini_cons + dias_treat_imp_sin_na_1 + escolaridad_rec + sus_principal_mod + freq_cons_sus_prin + condicion_ocupacional_corr+ via_adm_sus_prin_act+ origen_ingreso_mod + numero_de_hijos_mod + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + n_prev_off + n_off_vio + n_off_acq + n_off_sud + classification+ clas_r + porc_pobr, ties="efron", data= Base_fiscalia_v14)#[1:250,]

coxfit_more_var9 <- coxph(Surv(yrs_to_condemn,  event_r) ~ motivodeegreso_mod_imp_rec + ns(edad_al_ing_1, df=4) + edad_ini_cons + dias_treat_imp_sin_na_1 + escolaridad_rec + sus_principal_mod + freq_cons_sus_prin + condicion_ocupacional_corr+ via_adm_sus_prin_act+ origen_ingreso_mod + numero_de_hijos_mod + tenencia_de_la_vivienda_mod + dg_cie_10_rec + dg_trs_cons_sus_or + macrozona + n_prev_off + n_off_vio + n_off_acq + n_off_sud + classification+ clas_r + porc_pobr, ties="efron", data= Base_fiscalia_v14)#[1:250,]
...
ND <- with(Base_fiscalia_v14, data.frame(edad_al_ing_1 = seq(min(edad_al_ing_1),  max(edad_al_ing_1), length.out = 500),
  ...
  classification= rep(Mode(classification),500),
...
vars_cov<-c("edad_al_ing_1", "edad_ini_cons", "dias_treat_imp_sin_na_1", "escolaridad_rec", "sus_principal_mod", "via_adm_sus_prin_act", "cat_ocupacional_corr", "via_adm_sus_prin_act", "origen_ingreso_mod", "numero_de_hijos_mod", "tenencia_de_la_vivienda_mod", "dg_cie_10_rec", "dg_trs_cons_sus_or", "macrozona", "n_prev_off", "n_off_vio", "n_off_acq", "n_off_sud", "classification", "clas_r", "porc_pobr")
...
ND2 <- with(Base_fiscalia_v14, data.frame(edad_al_ing_1 = rep(quantile(edad_al_ing_1, .5, na.rm=T),3), 
  ...
  classification= rep(Mode(classification),3),
...
risk(coxfit_more_var8,ND2, .3)
risk(coxfit_more_var8,ND2, 1)
risk(coxfit_more_var8,ND2, 3)
risk(coxfit_more_var8,ND2, 5)
```
...
```{r stata1, echo=T, fig.align='center', message=T, error=T, eval=F}
...
if (grepl("CISS Fondecyt",rstudioapi::getSourceEditorContext()$path)==T){
    ...
    save.image(paste0(sub("2019","2022",sub("SUD_CL","",path)),"15.RData"))
  }
...
subset(Base_fiscalia_v14, select= c(c("hash_key", cont_vars_desc, cat_vars_desc, cat_vars_desc_off, vars_desc, "id_centro", "comuna_residencia_cod", "fech_ing_num_1", "fech_egres_imp", "cut_com_del", "cut_fec_nac","offender_d", "comuna_residencia_cod_rec", "porc_pobr", "Clasificación", "tipo_de_vivienda_mod"), "via_adm_sus_prin_act", "cat_ocupacional_corr", "classification", "clas_r")) %>% 
...
subset(Base_fiscalia_v14_pris, select= c(c("hash_key", cont_vars_desc, cat_vars_desc, cat_vars_desc_off, vars_desc, "id_centro", "comuna_residencia_cod", "fech_ing_num_1", "fech_egres_imp", "cut_com_del", "cut_fec_nac","offender_d", "comuna_residencia_cod_rec", "porc_pobr", "Clasificación", "tipo_de_vivienda_mod"), "via_adm_sus_prin_act", "cat_ocupacional_corr", "classification", "clas_r")) %>% 
  dplyr::arrange(hash_key, fech_ing_num_1) %>% 
     missRanger::missRanger(
                formula = . ~ . - hash_key,
                num.trees = 200, 
...
export_lab_stata_merge<-
  tibble::rownames_to_column(data.frame(Hmisc::label(dplyr::select(Base_fiscalia_v14,  all_of(c("hash_key", cont_vars_desc, cat_vars_desc, cat_vars_desc_off, vars_desc, "fech_ing_num_1", "fech_egres_imp", "cut_com_del", "cut_fec_nac","offender_d", "comuna_residencia_cod_rec", "porc_pobr", "Clasificación"))))))%>% data.frame() %>%
