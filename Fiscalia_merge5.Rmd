...
tbone_desc_merge52<-
CreateTableOne(vars=c(setdiff(vars_cov, "motivodeegreso_mod_imp_rec"), "offender_d"), data=  subset(Base_fiscalia_v14_pris, select=c(vars_cov, "offender_d")), factorVars = setdiff(vars_cov, c("motivodeegreso_mod_imp_rec","edad_al_ing_1", "edad_ini_cons","ano_nac_corr", "porc_pobr")), smd=T, strata="motivodeegreso_mod_imp_rec", addOverall = T, includeNA=T, test=T)
...
as.data.frame.TableOne(tbone_desc_merge52, smd=T, nonnormal= T)%>% 
  ...
  format_cells(1, 1:length(names(.)), "bold") %>%
  dplyr::select(-1) %>% 
  knitr::kable(size=10, format="markdown",caption= "Summary descriptives, Imprisonment(1), Found as an imputed (YES), by Baseline Treatment Status", escape=T)
#kable(size=10, format="html",caption= "Summary descriptives, by Baseline Treatment Status") %>%     kableExtra::kable_classic()
...
Base_fiscalia_v14%>% 
    ...
    dplyr::do(chisq.test(.$motivodeegreso_mod_imp_rec, .$measure) %>% broom::tidy()) %>%
    dplyr::mutate(p.value=ifelse(p.value<.001,"<0.001",sprintf("%1.3f",p.value))) %>% 
    dplyr::mutate(statistic=sprintf("%2.0f",statistic)) %>% 
    dplyr::mutate(report=paste0("X²(",parameter,", ",nrow(Base_fiscalia_v14),")=",statistic,"; p", ifelse(p.value=="<0.001",p.value, paste0("=",p.value)))) %>%
    dplyr::mutate(report=sub("0\\.","0,",sub("\\.",",",report))) %>% 
    rio::export("results.xlsx")

 chisq.test(Base_fiscalia_v14$motivodeegreso_mod_imp_rec, is.na(Base_fiscalia_v14$offender_d)) 
 chisq.test(Base_fiscalia_v14_pris$motivodeegreso_mod_imp_rec, is.na(Base_fiscalia_v14_pris$offender_d)) 

Base_fiscalia_v14 %>% 
    dplyr::select(setdiff(vars_cov, c("edad_al_ing_1", "edad_ini_cons","ano_nac_corr", "porc_pobr"))) %>% 
    tidyr::gather(variable,measure, -motivodeegreso_mod_imp_rec) %>% 
    dplyr::group_by(variable) %>%
    dplyr::do(fisher.test(.$motivodeegreso_mod_imp_rec, .$measure, workspace = 2e10, simulate.p.value = T, B=1e5) %>% broom::tidy()) %>% 
    rio::export("results2.xlsx")

fisher.test(Base_fiscalia_v14$motivodeegreso_mod_imp_rec, 
            is.na(Base_fiscalia_v14$offender_d), workspace = 2e10, simulate.p.value = T, B=1e5) 
fisher.test(Base_fiscalia_v14_pris$motivodeegreso_mod_imp_rec, 
            is.na(Base_fiscalia_v14_pris$offender_d), workspace = 2e10, simulate.p.value = T, B=1e5) 

# Kurksal Wallis
paste0("H(",kruskal.test(edad_al_ing_1 ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$parameter,")=",
       round(kruskal.test(edad_al_ing_1 ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$statistic,1), ", p=",
       round(kruskal.test(edad_al_ing_1 ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$p.value,3))

paste0("H(",kruskal.test(edad_ini_cons ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$parameter,")=",
       round(kruskal.test(edad_ini_cons ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$statistic,1), ", p=",
       round(kruskal.test(edad_ini_cons ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$p.value,3)) 

paste0("H(",kruskal.test(ano_nac_corr ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$parameter,")=",
       round(kruskal.test(ano_nac_corr ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$statistic,1), ", p=",
       round(kruskal.test(ano_nac_corr ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$p.value,3)) 

paste0("H(",kruskal.test(porc_pobr ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$parameter,")=",
       round(kruskal.test(porc_pobr ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$statistic,1), ", p=",
       round(kruskal.test(porc_pobr ~ motivodeegreso_mod_imp_rec, data=Base_fiscalia_v14)$p.value,3)) 
}
```
:::
...
rbindlist(list(list(c("Probs.",rep("",6))),cbind.data.frame(prob_imp_tab,prob_imp_diff_tab[,-1]),
               list(c("RMST",rep("",6))), cbind.data.frame(rmst_imp_tab,rmst_imp_diff_tab[,-1])), fill=T) %>% 
...
rbindlist(list(list(c("Probs.",rep("",6))),cbind.data.frame(prob_imp_tab_cc,prob_imp_diff_tab_cc[,-1]),
               list(c("RMST",rep("",6))), cbind.data.frame(rmst_imp_tab_cc,rmst_imp_diff_tab_cc[,-1])), fill=T) %>% 
    #dplyr::slice(-2,-3,-4,-5,-6,-7,-14,-15,-16,-17,-18,-19) %>%        
  dplyr::filter(nchar(Time)>=2) %>% 
  #knitr::kable("markdown", size=3, col.names= c("Time", "Complete Tr.","Late Disch.", "Early Disch.", "Comp. vs Late", "Comp. vs Early", "Early vs Late"), caption= "Offending with Condemnatory Sentence")
kbl(format = 'markdown',
...
rbindlist(list(list(c("Probs.",rep("",6))),cbind.data.frame(prob_imp_pris_tab_cc,prob_imp_diff_pris_tab_cc[,-1]), list(c("RMST",rep("",6))), cbind.data.frame(rmst_imp_pris_tab_cc,rmst_imp_diff_pris_tab_cc[,-1])), fill=T) %>% 
    dplyr::filter(nchar(Time)>=2)  %>% #rio::export("_tab_mariel_results.xlsx")
kbl(format = 'markdown',
        escape = FALSE,
        col.names= c("Time", "Complete Tr.","Late Disch.", "Early Disch.", "Comp. vs Late", "Comp. vs Early", "Early vs Late"), 
...
statadf_miss          <- rio::import("mariel_feb_23_m1.dta")
statadf_miss_pris     <- rio::import("mariel_feb_23_2_m1.dta")
...
logit3 <- glm (as.formula(paste("motivodeegreso_mod_imp_rec~", paste(vars, collapse = "+"))), data=statadf_main_cc, family=binomial(link= "logit"))

# statadf_main_cc_mod_cc<-
# dplyr::slice(statadf_main_cc,as.numeric(attr(predict(logit3, type="response"),"names")))

set.seed(2125)
W1 <- WeightIt::weightit(as.formula(paste("motivodeegreso_mod_imp_rec~", paste(vars, collapse = "+"))), data=statadf_main_cc, method = "bart", estimand = "ATE", use.mlogit = FALSE)
statadf_main_cc$weight_mult_b <- WeightIt::get_w_from_ps(W1$weights, statadf_main_cc$motivodeegreso_mod_imp_rec, estimand = "ATE", stabilize=T)
...
require(RMSTSens)
results.approx2 <- RMSTSens(time="diff", status="event", exposure="motivodeegreso_mod_imp_rec", level.exposed="1", ps="weight_mult_b", data=subset(statadf_main_cc,subset= motivodeegreso_mod_imp_rec!=3) |> dplyr::mutate(motivodeegreso_mod_imp_rec= ifelse(motivodeegreso_mod_imp_rec==2,1,0)), methods="Optim", use.multicore=TRUE, n.core=parallel::detectCores()-2, lambda=c(1,1.5), tau=3, ini.par=1, verbose=FALSE)
...
we compared against multi-valued treatments format.
...
cobalt::bal.tab(motivodeegreso_mod_imp_rec ~ tr_mod2+ sex_dum2+ edad_ini_cons+ esc1+ esc2+ sus_prin2+ sus_prin3+ sus_prin4+ sus_prin5+ fr_cons_sus_prin2+ fr_cons_sus_prin3+ fr_cons_sus_prin4+ fr_cons_sus_prin5+ cond_ocu2+ cond_ocu3+ cond_ocu4+ cond_ocu5+ cond_ocu6+ policonsumo+ num_hij2+ tenviv1+ tenviv2+ tenviv4+ tenviv5+ mzone2+ mzone3+ n_off_vio+ n_off_acq+ n_off_sud+ n_off_oth+ psy_com2+ psy_com3+ dep2+ rural2+ rural3+ porc_pobr+ susini2+ susini3+ susini4+ susini5+ ano_nac_corr+ cohab2+ cohab3+ cohab4+ fis_com2+ fis_com3+ rc_x1+ rc_x2+ rc_x3, data = statadf_main_cc,
                weights = list("Beyasian Additive Regression Trees" = "W1"),#, "RP(df=10)+ Motive of admission & No. of children"="rpdf10_m_nostag_or_ing_num_hij", "Gompertz+ Tenure status households"="gomp_m_nostag_ten_viv", "Gompertz+ Motive of admission & No. of children"="gomp_m_nostag_or_ing_num_hij", "RP(df=6)+ Tenure status households"="rpdf6_m_nostag_ten_viv", "RP(df=6)+ Motive of admission & No. of children"="rpdf6_m_nostag_or_ing_num_hij"),
                estimand = "ATE",
                which.treat = .all,
                un = T, 
                thresholds = c(cor = .2),
...
rbind(data.table::data.table(cbind(comp="Early Dropout vs. Tr. completion",bal_tab_2023_1$Pair.Balance$`2 vs. 1`$Balance), keep.rownames=T),
      data.table::data.table(cbind(comp="Late Dropout vs. Tr. completion",bal_tab_2023_1$Pair.Balance$`3 vs. 1`$Balance), keep.rownames=T),
      data.table::data.table(cbind(comp="Late vs. Early Dropout",bal_tab_2023_1$Pair.Balance$`3 vs. 2`$Balance), keep.rownames=T)) %>% 
  data.table::data.table(keep.rownames = T) %>% 
  dplyr::select(comp, rn, everything()) %>% 
  dplyr::mutate_if(is.numeric, ~round(.,2)) %>% 
  knitr::kable("markdown", caption="Balance Measures (multinomial response weights)")
...
rbind(cbind(comp="Early Dropout vs. Tr. completion",bal_tab_2023_1$Pair.Balance$`2 vs. 1`$Balance),
      cbind(comp="Late Dropout vs. Tr. completion",bal_tab_2023_1$Pair.Balance$`3 vs. 1`$Balance),
      cbind(comp="Late vs. Early Dropout",bal_tab_2023_1$Pair.Balance$`3 vs. 2`$Balance)) %>% 
    data.table::data.table(keep.rownames = T) %>% 
    dplyr::select(comp, rn, everything()) %>% 
    dplyr::mutate_if(is.numeric, ~round(.,2)) %>% 
    dplyr::summarise_if(is.numeric, ~list(min= min(.,na.rm=T), max= max(.,na.rm=T))) %>% t() %>%
...
dplyr::bind_rows(data.table::data.table(cbind(comp="Early Dropout vs. Tr. completion",bal_tab_2023_1$Pair.Balance$`2 vs. 1`$Observations),keep.rownames = T), data.table::data.table(cbind(comp="Early Dropout vs. Tr. completion",bal_tab_2023_1$Pair.Balance$`3 vs. 1`$Observations),keep.rownames = T), data.table::data.table(cbind(comp="Early Dropout vs. Tr. completion",bal_tab_2023_1$Pair.Balance$`3 vs. 2`$Observations),keep.rownames = T)) %>% 
  #dplyr::mutate_if(is.numeric, ~round(.,2)) %>% 
  knitr::kable("markdown", caption="Effective sample sizes")
```
:::
